...
if ( ! defined( 'SEARCHWP_WPML_VERSION' ) ) {
	define( 'SEARCHWP_WPML_VERSION', '1.6.9' );
}
...
class SearchWP_WPML {
	...
	function add_mods( $mods, $query ) {
		...
		foreach ( $sites as $site_id ) {
			...
			$mod->raw_join_sql( function( $runtime ) use ( $site_id, $wpdb_posts ) {
				global $wpdb;
			...
			foreach ( $query->get_engine()->get_sources() as $source ) {
				...
				$mod->raw_where_sql( function( $runtime_mod, $params ) use ( $post_type, $site_id, $wpdb_posts ) {
					$where_sql = $this->force_current_language( '1=1', $post_type, null, $wpdb_posts );
	 ...
	 * @return string
	 */
	function force_current_language( $sql, $post_type, $engine, $wpdb_posts = '' ) {
		global $wpdb, $sitepress;
		...
		if ( ! empty( $sitepress ) && method_exists( $sitepress, 'get_current_language' ) && method_exists( $sitepress, 'get_default_language' ) ) {
			...
			if ( $current_language == $default_language ) {
				$sql .= " AND ( ( {$wpdb_posts}.post_type != 'attachment' AND ( t.language_code = %s OR t.language_code IS NULL ) ) OR ( {$wpdb_posts}.post_type = 'attachment' AND t.language_code = %s ) ) ";
				...
				if ( method_exists( $sitepress, 'get_setting' ) ) {
					...
					if (
						...
						$sql .= " AND ( ( {$wpdb_posts}.post_type = %s AND ( t.language_code = %s OR t.language_code = %s OR t.language_code IS NULL ) ) ) ";
