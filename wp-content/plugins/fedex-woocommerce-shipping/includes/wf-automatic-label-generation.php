<?php
	...
	{
		$auto_label_generation = PH_WC_Fedex_Storage_Handler::ph_get_meta_data( $order_id, 'ph_fedex_auto_label_generation');
	...
	function wf_automatic_package_and_label_generation_fedex( $order_id )
	{
		...
		if( ! in_array($order_status, $allowed_order_status) ) {
			if( $shipping_setting_fedex['debug'] == 'yes' ) {
				WC_Admin_Meta_Boxes::add_error( __( "Since Order Status is ", 'ph-fedex-woocommerce-shipping' ).$order_status.__( ". Automatic label generation has been suspended (Fedex).", 'ph-fedex-woocommerce-shipping' ) );
			}

			PH_WC_Fedex_Storage_Handler::ph_add_and_save_meta_data($order_id, 'ph_fedex_auto_label_generation', 'failed');
		...
		if( empty($order_items) ) {
			WC_Admin_Meta_Boxes::add_error( __( 'Fedex - No product Found. Please check the products in order.', 'ph-fedex-woocommerce-shipping' ) );
			return;
		}
		...
		PH_WC_Fedex_Storage_Handler::ph_add_and_save_meta_data( $order_id, 'ph_fedex_auto_label_generation', '' );
		...
		// 	WC_Admin_Meta_Boxes::add_error( __( 'Fedex - Curl error while automatic package generation. Error number - ', 'ph-fedex-woocommerce-shipping' ). curl_errno($ch) );
		// }
		// curl_close($ch);
	}
	...
	function wf_auto_genarate_label_fedex($order_id,$package_data){
		...
		if( empty($package_data[key($package_data)]) ){
			...
			// @$output=curl_exec($ch);
			// if( ! $output && curl_errno($ch) ) {
			// 	WC_Admin_Meta_Boxes::add_error( __( 'Fedex - Curl error while automatic label generation. Error number - ', 'ph-fedex-woocommerce-shipping' ). curl_errno($ch) );
			// }
			// curl_close($ch);
		}
	}
	...
	{
		add_action('xa_fedex_label_generated_successfully','wf_after_label_generation_fedex',3,5);
	}


	function wf_after_label_generation_fedex($shipment_id,$encoded_label_image,$order_id,$shippinglabel_type = '', $return_label = false)
	{	

		$shipping_setting2 =get_option('woocommerce_wf_fedex_woocommerce_shipping_settings');

		$subject = ( isset($shipping_setting2['email_subject']) && !empty($shipping_setting2['email_subject']) ) ? $shipping_setting2['email_subject'] : __('Shipment Label For Your Order', 'ph-fedex-woocommerce-shipping').' [ORDER NO]';
		...
		if(!empty($shipment_id)){
			...
			if ( $return_label ) {
				...
				$img_url		= admin_url('/post.php?wf_fedex_viewReturnlabel='.base64_encode($shipment_id.'|'.$order_id));
			} else {
			...
			// Send ZPL Label Content in email body if enabled in settings
			if( ( isset( $shipping_setting2['zpl_in_email'] ) && $shipping_setting2['zpl_in_email'] == 'yes' ) && $shipping_setting2['image_type'] == 'zplii' ) {
	...
	{
		$shipment_id= PH_WC_Fedex_Storage_Handler::ph_get_meta_data($order_id,'wf_woo_fedex_shipmentId');
		if(!empty($shipment_id))
		{
			$img_url=admin_url('/post.php?wf_fedex_viewlabel='.base64_encode($shipment_id.'|'.$order_id));
