...
class PH_FedEx_Boxpack_Stack {
	...
	public function add_item( $length, $width, $height, $weight, $value = '', $meta = array() ) {
			$this->items[] = new PH_FedEx_Boxpack_Item_Stack( $length, $width, $height, $weight, $value, $meta );
	}
	...
	public function add_box( $length, $width, $height, $weight = 0 ) {
			$new_box       = new PH_FedEx_Boxpack_Box_Stack( $length, $width, $height, $weight );
			$this->boxes[] = $new_box;
			return $new_box;
	}
	...
	public function pack() {
		try {
			...
			if ( empty( $this->items ) || sizeof( $this->items ) == 0 ) {
				...
				$fedex_shipping_obj = new wf_fedex_woocommerce_shipping_method();
				$fedex_shipping_obj->debug( 'No items to pack!', 'notice' );
			}
...
/**
 * PH_FedEx_Boxpack_Box_Stack class.
 */
class PH_FedEx_Boxpack_Box_Stack {

	/** @var string ID of the box - given to packages */
	private $id = '';

	/** @var string Name of the box */
	private $name = '';
	...
	public function __construct( $length, $width, $height, $weight = 0 ) {
			...
			$stop_sorting = apply_filters( 'xa_fedex_stack_first_packing_stop_sorting_dimensions', true );
		if ( ! $stop_sorting ) {
			sort( $dimensions );
		}
...
/**
 * PH_FedEx_Boxpack_Item_Stack class.
 */
class PH_FedEx_Boxpack_Item_Stack {
	...
	public function __construct( $length, $width, $height, $weight, $value = '', $meta = array() ) {
			...
			$stop_sorting = apply_filters( 'xa_fedex_stack_first_packing_stop_sorting_dimensions', false );
		if ( ! $stop_sorting ) {
			sort( $dimensions );
		}
