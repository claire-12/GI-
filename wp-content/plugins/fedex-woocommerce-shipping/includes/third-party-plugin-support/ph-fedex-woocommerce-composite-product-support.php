	...
	function ph_fedex_handle_composite_products( $items, $order ) {
		...
		if( $composite_product_exist_in_order ) {
			...
			foreach( $order_items as $line_item ) {
				...
				$composite_weight 		= $line_item->get_meta('_composite_weight');
				$composite_parent 		= $line_item->get_meta('_composite_parent');
				$composite_children 	= $line_item->get_meta('_composite_children');


				// First Cond: For Comp Child Only
				// Second Cond: For Comp Parent Only
				// Third Cond: For Simple & Variable
				if (
					$shipped_individually === 'yes' ||
					(!empty($composite_weight) && !empty($composite_children)) ||
					(empty($composite_parent) && empty($composite_children))
				) {
