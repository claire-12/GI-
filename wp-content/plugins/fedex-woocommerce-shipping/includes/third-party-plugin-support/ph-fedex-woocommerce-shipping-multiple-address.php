...
function ph_fedex_shipping_packages_filter($packages,$address, $order_id){	
	...
	$wms_packages = PH_WC_Fedex_Storage_Handler::ph_get_meta_data($order_id, '_wcms_packages');	
...
function ph_fedex_update_shipping_method_instance_id($items, $that, $type )
{
	...
	if( $type[0] == 'shipping' && !empty($items) && is_array($items) )
	{
		...
		if( !empty($order_id) && !empty($shipping_methods) )
		{
			...
			{
				$order_shipping 			= PH_WC_Fedex_Storage_Handler::ph_get_meta_data($order_id, '_order_shipping', false);
				...
				{
					PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, '_order_total',  $new_order_total);
				}

				if( ($order_shipping != $shipping_total) && ($order_shipping < $shipping_total) )
				{
					PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, '_order_shipping', $shipping_total);
				}
