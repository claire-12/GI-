...
if( ! function_exists('ph_fedex_handle_bundle_products') ) {
	...
	function ph_fedex_handle_bundle_products( $package, $order) {
		...
		if( ! empty($deleted_products) && is_admin() && ! is_ajax() && class_exists('WC_Admin_Meta_Boxes') ) {
			WC_Admin_Meta_Boxes::add_error( __( "FedEx Warning! One or more Ordered Products have been deleted from the Order. Please check these Products- ", 'ph-fedex-woocommerce-shipping' ).implode( ',', $deleted_products ).'.' );
		}
		return $package;
	}
