...
class wf_fedex_woocommerce_shipping_admin_helper {
	 ...
	 * Version Variables
	 */
	public $rateservice_version, $ship_service_version, $pickup_service_version, $addressvalidationservice_version;
	/**
	 * Tracking ID Variables
	 */
	public $tracking_ids, $add_trackingpin_shipmentid;
	/**
	 * Settings
	 */
	public $settings;
	 ...
	 * General Variables
	 */
	public $is_international, $fed_req, $residential, $saturday_pickup, $working_days, $etd_label, $cod_collection_type, $signature_option, $exclude_tax, $timezone_offset, $dropoff_type, $customtotal, $charges_payment_type, $remove_special_char, $discounted_price, $invoice_commodity_value, $global_hs_code, $special_instructions, $payment_terms, $csb5_shipments, $ad_code, $gst_shipment, $csb_termsofsale, $under_bond, $meis_shipment, $saturday_delivery_label, $pro_forma_invoice, $document_content, $shipment_comments, $label_maskable_type;
	/**
	 * Soap Method
	 */
	public $soap_method;
	 ...
	 */
	public $weight_dimensions_manual, $dimension_unit, $weight_unit, $labelapi_dimension_unit, $labelapi_weight_unit;

	public $origin;
	/**
	 * Admin Details Variables
	 */
	public $account_number, $meter_number, $smartpost_hub;

	public $indicia;
	/**
	 * API Details Variables
	 */
	public $api_key, $api_pass;
	/**
	 * General Variables
	 */
	public $ship_from_address, $production, $debug, $insure_contents, $min_order_amount_for_insurance, $request_type;
	/**
	 * Packing Variables
	 */
	public $packing_method, $box_max_weight, $weight_pack_process, $boxes, $packaging_type, $mode, $stack_to_volume, $skip_products;

	/**
	 * Rate Variables
	 */
	public $offer_rates, $duties_and_taxes_rate, $fedex_conversion_rate, $fedex_one_rate, $fedex_one_rate_package_ids;
	 ...
	 * Freight Variables
	 */
	public $freight_enabled, $freight_class, $freight_number, $freight_bill_street, $freight_billing_street_2, $freight_billing_city, $freight_billing_state, $freight_billing_postcode, $freight_billing_country, $freight_shipper_person_name, $freight_shipper_company_name, $freight_shipper_phone_number, $frt_shipper_street, $freight_shipper_street_2, $freight_shipper_city, $freight_shipper_residential, $freight_document_type;
	/**
	 * Label Type and Format
	 */
	public $output_format, $image_type;
	/**
	 * Commercial invoice Variables
	 */
	public $commercial_invoice, $commercial_invoice_shipping, $commercial_invoice_order_currency;
	/**
	 * USMCA Variables
	 */
	public $usmca_certificate, $usmca_ci_certificate_of_origin;
	/**
	 * Blanket Variables
	 */
	public $blanket_begin_period, $blanket_end_period;
	/**
	 * Specification Variables
	 */
	public $certifier_specification, $importer_specification, $producer_specification;
	/**
	 * Dry Ice Variables
	 */
	public $is_dry_ice_enabled, $dry_ice_shipment, $dry_ice_total_weight;
	/**
	 * Shipping Payor Variables
	 */
	public $shipping_payor_acc_no, $shipping_payor_cname, $shipp_payor_company, $shipping_payor_phone, $shipping_payor_email, $shipp_payor_address1, $shipp_payor_address2, $shipping_payor_city, $shipping_payor_state, $shipping_payor_zip, $shipp_payor_country;
	/**
	 * Customs Variables
	 */
	public $customs_duties_payer, $customs_ship_purpose;
	/**
	 * Email Variables
	 */
	public $email_notification, $shipper_email;
	/**
	 * Broker Variables
	 */
	public $broker_acc_no, $broker_name, $broker_company, $broker_phone, $broker_email, $broker_address, $broker_city, $broker_state, $broker_zipcode, $broker_country;
	/**
	 * Tin Variables
	 */
	public $tin_number;
	/**
	 * Alt Address Variables
	 */
	public $alternate_return_address, $billing_as_alternate_return_address, $alt_return_person_name, $alt_return_company_name, $alt_return_phone_number, $alt_return_streetline, $alt_return_city, $alt_return_postcode, $alt_return_country, $alt_return_state;
	/**
	 * Hazmat Variables
	 */
	public $hazmat_package_enabled, $hazmat_package_type, $hazmat_package_material;
	/**
	 * International Variables
	 */
	public $int_return_label_reason, $int_return_label_desc, $internationalPakcagingFromUs, $internationalPakcagingFromNonUs;
	/**
	 * Currency Variables
	 */
	public $wc_store_currency, $fedex_currency;
	/**
	 * Doc Tab Variables
	 */
	public $doc_tab_content, $doc_tab_orientation;
	/**
	 ...
	 */
	public $third_party_acc_no, $thirdpartyConsignee;
	/**
	...
	/**
	 * Domestic Packaging Variables
	 */
	public $domesticPackagingForUS, $domesticPackagingForNonUS;
	/**
	 * Order Variables
	 */
	public $order_id, $order_object, $order;
	/**
	 * Home Delivery Variables
	 */
	public $custom_services, $default_dom_service, $fedex_ltl_freight_services, $pre_service;
	/**
	 * Master Tracking ID
	 */
	public $master_tracking_id;
	/**
	 * Shipment ID
	 */
	public $shipmentId;
	...
	public $splCharToFind    = array( 'é', 'á', 'ä', 'Ä', 'Ã', 'ö', 'Ö', 'ü', 'Ü', 'ß', 'б', 'в', 'г', 'д', 'ё', 'ж', 'з', 'и', 'й', 'к', 'л', 'м', 'н', 'п', 'т', 'ф', 'х', 'ц', 'ч', 'ш', 'щ', 'ъ', 'ы', 'ь', 'э', 'ю', 'я', 'А', 'Б', 'Г', 'Д', 'Ё', 'Ж', 'З', 'И', 'Й', 'Л', 'П', 'У', 'Ф', 'Ц', 'Ч', 'Ш', 'Щ', 'Ъ', 'Ы', 'Ь', 'Э', 'Ю', 'Я', '№', 'ę', 'ł', 'ń', 'ś', 'ź', 'ż', 'ď', 'ě', 'ň', 'ř', 'š', 'ť', 'ů', 'ž', 'č', 'ő', 'ű', 'α', 'β', 'γ', 'δ', 'ε', 'ζ', 'η', 'θ', 'ι', 'κ', 'λ', 'μ', 'ν', 'ξ', 'ο', 'π', 'ρ', 'σ', 'ς', 'τ', 'υ', 'φ', 'χ', 'ψ', 'ω', 'ă', 'ș', 'ț', 'Č', 'Š', 'Ž', 'đ', 'Č', 'Ć', 'Đ', 'Š', 'Ž', 'Č', 'Ć', 'Đ', 'Š', 'Ž', 'ā', 'ē', 'ī', 'ķ', 'ļ', 'ņ', 'ū', 'ċ', 'ġ', 'ħ', 'ş', 'ţ', 'є', 'ї', 'ў', 'ė', 'į', 'ų', 'ѓ', 'ѕ', 'ј', 'њ', 'ќ', 'џ' );
	public $splCharToReplace = array( 'e', 'a', 'a', 'A', 'A', 'o', 'O', 'u', 'U', 'B', 'b', 'v', 'g', 'd', 'io', 'zh', 'z', 'i', 'y', 'k', 'l', 'm', 'n', 'p', 't', 'f', 'h', 'ts', 'ch', 'sh', 'sht', 'a', 'i', 'y', 'e', 'yu', 'ya', 'A', 'B', 'G', 'D', 'Io', 'Zh', 'Z', 'I', 'Y', 'L', 'P', 'U', 'F', 'Ts', 'Ch', 'Sh', 'Sht', 'A', 'I', 'Y', 'e', 'Yu', 'Ya', 'No.', 'e', 'l', 'n', 's', 'z', 'z', 'd', 'e', 'n', 'r', 's', 't', 'u', 'z', 'c', 'o', 'u', 'a', 'b', 'g', 'd', 'e', 'z', 'i', 'th', 'i', 'k', 'l', 'm', 'n', 'x', 'o', 'p', 'r', 's', 's', 't', 'u', 'f', 'ch', 'ps', 'o', 'a', 's', 't', 'C', 'S', 'Z', 'dj', 'C', 'C', 'Dj', 'S', 'Z', 'C', 'C', 'Dj', 'S', 'Z', 'a', 'e', 'i', 'k', 'l', 'n', 'u', 'c', 'g', 'h', 's', 't', 'ye', 'yi', 'w', 'e', 'i', 'u', 'gj', 'dz', 'j', 'nj', 'kj', 'dz' );

	// PDS-179
	public $prioritizedSignatureOption = array(
		5 => 'ADULT',
		4 => 'DIRECT',
		3 => 'INDIRECT',
		2 => 'SERVICE_DEFAULT',
		1 => 'NO_SIGNATURE_REQUIRED',
		0 => '',
	);
	...
	private function init() {
		...
		if ( $this->soap_method == 'nusoap' && ! class_exists( 'nusoap_client' ) ) {
			include_once plugin_dir_path( __DIR__ ) . 'includes/nusoap/lib/nusoap.php';
		}

		if ( ! class_exists( 'PH_WC_Fedex_Storage_Handler' ) ) {
			include_once plugin_dir_path( __DIR__ ) . 'class-ph-wc-fedex-storage-handler.php';
		}
		...
		$this->mode                       = isset( $this->settings['packing_algorithm'] ) && ! empty( $this->settings['packing_algorithm'] ) ? $this->settings['packing_algorithm'] : 'volume_based';

		$this->stack_to_volume = ( isset( $this->settings['stack_to_volume'] ) && $this->settings['stack_to_volume'] == 'yes' ) ? true : false;

		$this->skip_products = ( isset( $this->settings['skip_products'] ) && ! empty( $this->settings['skip_products'] ) ) ? $this->settings['skip_products'] : array();
		...
		$this->internationalPakcagingFromUs = array(
		...
		$this->internationalPakcagingFromNonUs = array(
			'FEDEX_10KG_BOX'        => 'FEDEX_10KG_BOX',
			'FEDEX_25KG_BOX'        => 'FEDEX_25KG_BOX',
		...
		$this->domesticPackagingForUS = array(
			'FEDEX_SMALL_BOX'       => 'FEDEX_SMALL_BOX',
			'FEDEX_MEDIUM_BOX'      => 'FEDEX_MEDIUM_BOX',
			'FEDEX_LARGE_BOX'       => 'FEDEX_LARGE_BOX',
			'FEDEX_EXTRA_LARGE_BOX' => 'FEDEX_EXTRA_LARGE_BOX',
		...
		$this->domesticPackagingForNonUS = array(
			'FEDEX_SMALL_BOX'       => 'FEDEX_BOX',
			'FEDEX_MEDIUM_BOX'      => 'FEDEX_BOX',
			'FEDEX_LARGE_BOX'       => 'FEDEX_BOX',
			'FEDEX_EXTRA_LARGE_BOX' => 'FEDEX_BOX',
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			foreach ( $additional_products as $values ) {
				if ( ! $values['data']->needs_shipping() ) {
					$this->debug( sprintf( __( 'Product #%d is virtual. Skipping.', 'ph-fedex-woocommerce-shipping' ), $item_id ), 'error' );
				...
				if ( ! $values['data']->get_weight() ) {
					$this->debug( sprintf( __( 'Product # is missing weight. Aborting.', 'ph-fedex-woocommerce-shipping' ), $item_id ), 'error' );
	...
	private function box_shipping( $package ) {
		...
		volume_based:
		if ( $this->mode == 'stack_first' ) {

			if ( ! class_exists( 'PH_FedEx_Boxpack_Stack' ) ) {
				include_once 'class-ph-packing-stack.php';
			}
			$boxpack = new PH_FedEx_Boxpack_Stack();
		} else {
		...
		foreach ( $this->boxes as $key => $box ) {
			...
			if ( isset( $box['name'] ) && ! empty( $box['name'] ) ) {
				$newbox->setName( $box['name'] );
			} else {
				$newbox->setName( 'Custom Box' );
			}
			...
			if ( $this->mode == 'stack_first' ) {
				...
				$newbox = $boxpack->add_box( $box['height'], $box['width'], $box['length'], $box['box_weight'] );
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			foreach ( $additional_products as $values ) {
				...
				if ( ! empty( $pre_packed ) && $pre_packed == 'yes' ) {
					...
					$this->debug( sprintf( __( 'Pre Packed product. Skipping the product ' . $values['data']->id, 'ph-fedex-woocommerce-shipping' ), $item_id ) );
				...
				if ( $dimensions[0] && $dimensions[1] && $dimensions[2] && $weight ) {
					...
					$this->debug( sprintf( __( 'Product #%s is missing weight or dimensions. Aborting.', 'ph-fedex-woocommerce-shipping' ), $item_id ), 'error' );
		...
		$stop_fallback = apply_filters( 'ph_fedex_stop_fallback_from_stack_first_to_vol_based', false );

		if ( $this->mode == 'stack_first' && ! $stop_fallback && $this->stack_to_volume ) {

			foreach ( $packages as $key => $package_value ) {
				$box_volume                 = $package_value->length * $package_value->width * $package_value->height;
				$box_used_volume            = isset( $package_value->volume ) && ! empty( $package_value->volume ) ? $package_value->volume : 1;
				...
				if ( isset( $box_used_volume_percentage ) && $box_used_volume_percentage < 44 ) {
					...
					// Add by Default
					$this->admin_diagnostic_report( '(FALLBACK) : Stack First Method changed to Volume Based. Reason: Selected Box Volume % used is less than 44%' );
	...
	private function xa_get_box_packages( $packages, $group_id = 1 ) {
		...
		foreach ( $packages as $package ) {
			...
			sort( $dimensions );
			$box_name = isset( $package->name ) && ! empty( $package->name ) ? $package->name : '';
			...
			$group = array(
				...
				'Weight'            => array(
					...
					'Value' => $this->round_up( ( ! empty( $package->weight ) ? $package->weight : 0 ), 2 ),
					'Units' => $this->labelapi_weight_unit,
				),
	...
	private function weight_based_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			foreach ( $additional_products as $values ) {
				...
				if ( ! empty( $product_weight ) ) {
					...
					$product_weight = $this->round_up( ( ! empty( $product_weight ) ? $product_weight : 0 ), 2 );
					...
					$this->debug( sprintf( __( 'Product #%s is missing weight. Aborting.', 'ph-fedex-woocommerce-shipping' ), $item_id ), 'error' );
		...
		if ( ! empty( $pre_packed_contents ) ) {
			$prepacked_requests = $this->wf_fedex_add_pre_packed_product( $pre_packed_contents, $group_id, $freight_id );
			$to_ship            = array_merge( $to_ship, $prepacked_requests );
		}
		return $to_ship;
	}
	...
	private function wf_fedex_add_pre_packed_product( $pre_packed_items, $group_id = 1, $freight_id = 1 ) {
	...
	/**
	 * Round the given number according to the precision
	 *
	 * @param float $value
	 * @param int   $precision
	 * @param float $rounded_value
	 */
	private function round_up( $value, $precision = 2 ) {

		$rounded_value = round( $value, $precision );

		if ( empty( $rounded_value ) ) {

			$pow           = pow( 10, $precision );
			$rounded_value = ( ceil( $pow * $value ) + ceil( $pow * $value - ceil( $pow * $value ) ) ) / $pow;
		}
		return $rounded_value;
	}

	private function wf_create_soap_client( $wsdl, $options = array( 'trace' => true ) ) {
	...
	public function residential_address_validation( $package ) {
		...
		if ( $this->production && in_array( $package['destination']['country'], $this->address_validation_contries ) ) {
			...
			try {
				...
				$request['Version']                 = array(
					'ServiceId'    => 'aval',
					'Major'        => $this->addressvalidationservice_version,
					'Intermediate' => '0',
					'Minor'        => '0',
				);
				...
				$wsdl_dir = plugin_dir_path( __DIR__ ) . 'fedex-wsdl/production/AddressValidationService_v' . $this->addressvalidationservice_version . '.wsdl';
				...
				if ( Ph_Fedex_Woocommerce_Shipping_Common::phIsNewRegistration() ) {
					...
					if ( ! Ph_Fedex_Woocommerce_Shipping_Common::phHasActiveLicense() ) {
						...
						$this->admin_diagnostic_report( '----------------------------- FEDEX ADDRESS VALIDATION -----------------------------' );
						...
						$proxyParams = Ph_Fedex_Woocommerce_Shipping_Common::phGetParamsForProxyCall( $apiAccessDetails, $request, 'validate_address' );

						$client = $this->wf_create_soap_client( $wsdl_dir, $proxyParams['options'] );
				...
				if ( WF_FEDEX_ADV_DEBUG_MODE == 'on' ) { // Test mode is only for development purpose.
					...
					$this->debug( 'FedEx ADDRESS VALIDATION REQUEST: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( htmlspecialchars( $xml_request ), true ) . "</pre>\n" );
					$this->debug( 'FedEx ADDRESS VALIDATION RESPONSE: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( htmlspecialchars( $xml_response ), true ) . "</pre>\n" );

					$this->admin_diagnostic_report( '----------------------------- FedEx Address Validation XML Request -----------------------------' );
					$this->admin_diagnostic_report( htmlspecialchars( $xml_request ) );
					$this->admin_diagnostic_report( '----------------------------- FedEx Address Validation XML Response -----------------------------' );
					$this->admin_diagnostic_report( htmlspecialchars( $xml_response ) );
				}

				if ( isset($response->HighestSeverity) && $response->HighestSeverity == 'SUCCESS' ) {
				...
				$this->debug( __( 'SoapFault while residential_address_validation.', 'ph-fedex-woocommerce-shipping' ) );
			}
		}
		...
		if ( $this->residential == false ) {
			$this->debug( __( 'Business Address', 'ph-fedex-woocommerce-shipping' ) );
		}
	}
	...
	private function get_fedex_api_request( $fedex_packages, $package, $request_type ) {
		...
		} elseif ( $this->origin_country == 'US' && $package['destination']['country'] != 'US' ) {

			$request['RequestedShipment']['PackagingType'] = isset( $this->internationalPakcagingFromUs[ $this->packaging_type ] ) ? $this->internationalPakcagingFromUs[ $this->packaging_type ] : 'YOUR_PACKAGING';

		} elseif ( $this->origin_country != 'US' && ( $package['destination']['country'] != $this->origin_country ) ) {

			$request['RequestedShipment']['PackagingType'] = isset( $this->internationalPakcagingFromNonUs[ $this->packaging_type ] ) ? $this->internationalPakcagingFromNonUs[ $this->packaging_type ] : 'YOUR_PACKAGING';

		} elseif ( $this->origin_country == 'US' && $package['destination']['country'] == 'US' ) {

			$request['RequestedShipment']['PackagingType'] = isset( $this->domesticPackagingForUS[ $this->packaging_type ] ) ? $this->domesticPackagingForUS[ $this->packaging_type ] : 'YOUR_PACKAGING';

		} elseif ( $this->origin_country != 'US' && ( $this->origin_country == $package['destination']['country'] ) ) {

			$request['RequestedShipment']['PackagingType'] = isset( $this->domesticPackagingForNonUS[ $this->packaging_type ] ) ? $this->domesticPackagingForNonUS[ $this->packaging_type ] : 'YOUR_PACKAGING';
		...
		// To support FedEx Multi-warehouse addon
		$shipping_charges_payment = apply_filters( 'ph_shipping_charges_payment_details', $shipping_charges_payment, $package );
	...
	public function skip_products( $package ) {
		...
		foreach ( $package['contents'] as $product_key => $product ) {

			$product_shipping_class = $product['data']->get_shipping_class();

			if ( in_array( $product_shipping_class, $this->skip_products ) ) {

				$skipped_products[] = $product['data']->get_id();

				unset( $package['contents'][ $product_key ] );
			}
		}
	...
	public function get_fedex_requests( $fedex_packages, $package, $request_type = '' ) {
		...
		if ( $fedex_packages ) {
			...
			// Check Order Contain atleast one Hazardous Material
			$isHazardousMaterialsInPackage = false;
			...
			foreach ( $fedex_packages as $key => $parcel ) {
				...
				if ( isset( $parcel['packed_products'] ) && $request_type === 'freight' ) {
					...
					$box_weight         = isset( $parcel['box_weight'] ) ? $parcel['box_weight'] : 0;
					...
					foreach ( $parcel['packed_products'] as $product ) {
						...
						*/
						...
						$option = get_post_meta( $product_id, '_ph_fedex_dg_option', 1 );

						if ( $option == 'HAZARDOUS_MATERIALS' ) {

							$isHazardousMaterialsInPackage = true;
							$hazmat_id_num                 = get_post_meta( $product_id, '_ph_fedex_hp_id_num', 1 );
							$hazmat_packaging_group        = get_post_meta( $product_id, '_ph_fedex_hp_packaging_group', 1 );
							$hazmat_proper_shipping_name   = get_post_meta( $product_id, '_ph_fedex_hp_proper_shipping_name', 1 );
							$hazmat_hazard_class           = get_post_meta( $product_id, '_ph_fedex_hp_hazard_class', 1 );
							$hazmat_subsidairy_class       = get_post_meta( $product_id, '_ph_fedex_hp_subsidiary_classes', 1 );
							$hazmat_label_text             = get_post_meta( $product_id, '_ph_fedex_hp_label_text', 1 );

							// Adding DANGEROUS_GOODS option type in LineItems
							$line_items['HazardousMaterials'] = $option;

							// Adding DANGEROUS_GOODS details in LineItems Description
							$line_items['Description'] = $hazmat_id_num . ', ' . $hazmat_packaging_group . ', ' . $hazmat_proper_shipping_name . ', ' . $hazmat_hazard_class . ', ' . $hazmat_subsidairy_class . ', ' . $hazmat_label_text;
						}

						$freight_line_items[] = $line_items;
					}

					// Error: The total of Requested Package Line Item Weights cannot exceed the Requested Shipment Total Weight.
					// Reason: In Box Packing Algorithm, when Box has it's own weight then total freight line items weight will not match with Package Weight
					// Resolution: Add Box Weight as seperate Fright Line Items in order to match the Total Package Weight
					if ( ! empty( $box_weight ) ) {

						$freight_class = $this->freight_class < 100 ? '0' . $this->freight_class : $this->freight_class;
						$freight_class = 'CLASS_' . str_replace( '.', '_', $freight_class );
						...
						$freight_line_items[] = array(
							...
							'FreightClass'  => $freight_class,
							'Packaging'     => 'SKID',
							'Description'   => 'Freight Box Weight',
							'Weight'        => array(

								'Units' => $this->labelapi_weight_unit,
								'Value' => $this->round_up( $box_weight, 2 ),
							),
						);
					}
				}
			...
			foreach ( $fedex_packages as $key => $parcel ) {
				...
				for ( $i = 0; $i < $num_of_packages; $i++ ) {  // looping packages based on no: of packages selected in order page
					...
					if ( ! empty( $parcel['service'] ) ) {
						...
						if ( $parcel['service'] == 'INTERNATIONAL_ECONOMY_FREIGHT' || $parcel['service'] == 'INTERNATIONAL_PRIORITY_FREIGHT' ) {

							$request['RequestedShipment']['ExpressFreightDetail']['ShippersLoadAndCount'] = count( $fedex_packages );

							$request['RequestedShipment']['ExpressFreightDetail']['BookingConfirmationNumber'] = isset( $_GET['bcn'] ) ? $_GET['bcn'] : '';
						}
					}

					$parcel_value = (float) $parcel['InsuredValue']['Amount'] * (int) $parcel['GroupPackageCount'];
						...
						if ( $parcel_request['packed_products'] ) {
							foreach ( $parcel_request['packed_products'] as $product ) {
								...
								if ( ( empty( $this->invoice_commodity_value ) && $this->discounted_price ) || $this->invoice_commodity_value == 'discounted_price' ) {
									$order_object = wc_get_order( $this->order_id );
								...
								$custom_declared_value = current( $this->xa_get_custom_product_option_details( array( $product ), '_wf_fedex_custom_declared_value' ) );
								...
								if ( $is_dry_ice_product == 'yes' ) {
									...
									$this->dry_ice_total_weight += $this->round_up( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_weight( $dry_ice_weight, 'kg' ) * $parcel['GroupPackageCount'], 2 ); // Fedex support dry ice weight in KG only
								}
								...
								$wf_hs_code = $this->xa_get_custom_product_option_details( $parcel['packed_products'], '_wf_hs_code' );
								...
								if ( ! empty( $wf_hs_code ) ) {
									$commodoties[ $main_product_id ]['HarmonizedCode'] = current( $wf_hs_code );
								} elseif ( ! empty( $this->global_hs_code ) ) {
					...
					if ( isset( $_GET['sat_delivery'] ) ) {

						PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $this->order_id, 'ph_fedex_sat_delivery_option_meta', $_GET['sat_delivery'] );
					}
					...
					if ( 'freight' === $request_type ) {
						...
						if ( $this->fedex_one_rate && '' === $request_type && in_array( $request['RequestedShipment']['PackagingType'], $this->fedex_one_rate_package_ids ) && count( $fedex_packages ) == 1 ) {
						...
						$order_payment_method = $this->order->get_payment_method();
						if ( ( isset( $_GET['cod'] ) && $_GET['cod'] === 'true' ) || ( ! isset( $_GET['cod'] ) && $order_payment_method == 'cod' ) ) {
							if ( ! ( $parcel['service'] == 'FEDEX_GROUND' || $parcel['service'] == 'GROUND_HOME_DELIVERY' ) ) {
								...
								if ( $this->cod_collection_type == 'PERSONAL_CHECK' || $this->cod_collection_type == 'COMPANY_CHECK' || ( $package['destination']['country'] == 'IN' && $this->convert_to_inr_currency( $order_total ) >= 20000 ) ) {
									...
									$phonenummeta = $this->order->get_shipping_phone();
									$phonenum     = ! empty( $phonenummeta ) ? $phonenummeta : $this->order->get_billing_phone();
						...
						$ph_sat_checked = PH_WC_Fedex_Storage_Handler::ph_get_meta_data( $this->order_id, 'ph_fedex_sat_delivery_option_meta' );

						// $request['RequestedShipment']['SpecialServicesRequested']['SignatureOptionDetail']['OptionType']='SERVICE_DEFAULT';
						if ( $ph_sat_checked == 'yes' || ( $ph_sat_checked == false && $this->saturday_delivery_label && ! isset( $_GET['sat_delivery'] ) ) ) {
						...
						switch ( $this->email_notification ) {
							case 'CUSTOMER':
								...
								if ( $receipient_customer ) {
									$event_notification[] = array(
										...
										'NotificationDetail' => $receipient_customer,
							...
							case 'SHIPPER':
								...
								if ( $receipient_shipper ) {
									$event_notification[] = array(
										'Role'   => 'SHIPPER',
										'Events' => $events_requested,
										'NotificationDetail' => $receipient_shipper,
										'FormatSpecification' => array( 'Type' => 'HTML' ),
							...
							case 'BOTH':
									$receipient_customer = $this->notification_receiver( $billing_email, 'RECIPIENT' );
									$receipient_shipper  = $this->notification_receiver( $this->shipper_email, 'SHIPPER' );
								if ( $receipient_customer && $receipient_shipper ) {
									$event_notification[] = array(
										'Role'   => 'SHIPPER',
										'Events' => $events_requested,
										'NotificationDetail' => $receipient_shipper,
						...
						if ( is_array( $event_notification ) && sizeof( $event_notification ) > 0 ) {
							$special_servicetypes[] = 'EVENT_NOTIFICATION';

							$request['RequestedShipment']['SpecialServicesRequested']['EventNotificationDetail']['EventNotifications'] = $event_notification;
					...
					$order_subtotal = is_object( $order_object ) ? $order_object->get_subtotal() : 0;

					if ( ! $this->insure_contents || 'smartpost' === $request_type || empty( $parcel['InsuredValue']['Amount'] ) || $this->min_order_amount_for_insurance > $order_subtotal ) {

						unset( $parcel_request['InsuredValue'] );

						if ( $this->insure_contents && 'smartpost' != $request_type ) {

							$this->admin_diagnostic_report( 'Insurance eligibility criteria not met - Order subtotal must be greater or equal to Min Order Amount' );
						}
					}
					if ( 'smartpost' === $request_type ) {
					...
					if ( isset( $_GET['bcn'] ) ) {
						...
						PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $this->order_id, 'ph_fedex_booking_confirmation_num_meta', $_GET['bcn'] );
					}

					// PDS-179
					if ( 'smartpost' !== $request_type ) {

						$is_skiped_signature = true;

						if ( isset( $_GET['signature_option'] ) && ! empty( $_GET['signature_option'] ) ) {

							if ( $request_type != 'freight' || ( $request_type == 'freight' && $_GET['signature_option'] == 'SERVICE_DEFAULT' ) ) {

									$is_skiped_signature                                  = false;
									...
									PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $this->order_id, 'ph_fedex_signature_option_meta', array_search( $_GET['signature_option'], $this->prioritizedSignatureOption ) );
							}
						} elseif ( ! isset( $_GET['signature_option'] ) ) {
							...
							if ( ! empty( $bulk_label_signature ) && ( $request_type != 'freight' || ( $request_type == 'freight' && $_GET['signature_option'] == 'SERVICE_DEFAULT' ) ) ) {
						...
						if ( $request_type == 'freight' && $is_skiped_signature && isset( $_GET['signature_option'] ) && ! empty( $_GET['signature_option'] ) ) {

							$this->admin_diagnostic_report( 'Not Eligible for Signature - Freight Shipment' );
							$this->admin_diagnostic_report( print_r( $_GET['signature_option'], 1 ) );
						}
					}
					...
					$dgOptions       = $this->xa_get_custom_product_option_details( $parcel['packed_products'], '_ph_fedex_dg_option' );

					if ( isset( $dgOptions ) && ! empty( $dgOptions ) ) {

						$dgOptionsType = '';

						// dgOptions will be an array, so multiple product with different option, then taking first product's option.
						foreach ( $dgOptions as $value ) {
					...
					} elseif ( ! empty( $dangerous_goods ) && empty( $hazmat_products ) ) {

						$dgOptionsType = 'LIMITED_QUANTITIES_COMMODITIES';

					} elseif ( ! empty( $hazmat_products ) ) {

						$dgOptionsType = 'HAZARDOUS_MATERIALS';
						$dgOptions     = $hazmat_products;
					}

					if ( ! empty( $dgOptionsType ) && $request_type != 'freight' && ( $dgOptionsType == 'ORM_D' ) ) {
					...
					if ( ! empty( $dgOptionsType ) && $request_type != 'freight' && ( $dgOptionsType == 'LIMITED_QUANTITIES_COMMODITIES' ) ) {
						...
						$dangerous_goods_cargo_aircraft_only = $this->xa_get_custom_product_option_details( $parcel['packed_products'], '_ph_fedex_dg_cargo_aircraft_only' );

						$line_items_special_services['SpecialServiceTypes'][] = 'DANGEROUS_GOODS';
						$line_items_special_services['DangerousGoodsDetail']  = array(
							'CargoAircraftOnly' => isset( $dangerous_goods_cargo_aircraft_only ) && ! empty( $dangerous_goods_cargo_aircraft_only ) ? true : false,
							...
							'Options'           => 'LIMITED_QUANTITIES_COMMODITIES',
						);
					}

					if ( ! empty( $dgOptionsType ) && $request_type != 'freight' && ( $dgOptionsType == 'SMALL_QUANTITY_EXCEPTION' ) ) {

						$line_items_special_services['SpecialServiceTypes'][] = 'DANGEROUS_GOODS';

						$line_items_special_services['DangerousGoodsDetail'] = array(
							'Options' => 'SMALL_QUANTITY_EXCEPTION',
						);
					}

					// For freight request SpecialServiceTypes DANGEROUS_GOODS should be pass in Shipment Level
					if ( isset( $dgOptionsType ) && ! empty( $dgOptionsType ) && $request_type != 'freight' && ( $dgOptionsType == 'HAZARDOUS_MATERIALS' || $dgOptionsType == 'BATTERY' ) ) {
						...
						$line_items_special_services['SpecialServiceTypes'][]           = 'DANGEROUS_GOODS';
						$line_items_special_services['DangerousGoodsDetail']['Options'] = $dgOptionsType;

						$i = 0;

						foreach ( $dgOptions as $product_key => $value ) {

							if ( $value != 'HAZARDOUS_MATERIALS' && $value != 'BATTERY' ) {
									continue;
							}
							...
							foreach ( $parcel['packed_products'] as $key => $product ) {
								if ( $product_key == $product->id || $product_key == $product->variation_id ) {
									$product_weight += $this->round_up( ( ! empty( $product->get_weight() ) ? $product->get_weight() : 0 ), 2 );
								}
							}
					...
					// Battery without dangerous goods
					$batteryProducts = $this->xa_get_custom_product_option_details( $parcel['packed_products'], '_battery_products' );

					if ( ! empty( $batteryProducts ) ) {

						$batteryMaterialType = $this->xa_get_custom_product_option_details( $parcel['packed_products'], '_ph_fedex_battery_material_type' );
						$batteryPackingType  = $this->xa_get_custom_product_option_details( $parcel['packed_products'], '_ph_fedex_battery_packing_type' );

						$line_items_special_services['SpecialServiceTypes'][] = 'BATTERY';

						foreach ( $batteryProducts as $productKey => $value ) {

							$line_items_special_services['BatteryDetails'] = array(
								'Material'          => ! empty( $batteryMaterialType[ $productKey ] ) ? $batteryMaterialType[ $productKey ] : '',
								'Packing'           => ! empty( $batteryPackingType[ $productKey ] ) ? $batteryPackingType[ $productKey ] : '',
					...
					if ( $this->is_international && $this->csb5_shipments ) {
						...
						// CSB Dept Num
						$csbTermsOfSale = ( ! empty( $this->csb_termsofsale ) && ( $this->csb_termsofsale != 'none' ) ) ? $this->csb_termsofsale : 'FOB';
						$dept_num       = 'CSB5/' . $this->gst_shipment . '/' . $csbTermsOfSale . '/' . $this->under_bond . '/E/' . $this->meis_shipment . '/' . $gst . '/' . $invoice_date;
					...
					} elseif ( $this->insure_contents && $this->min_order_amount_for_insurance <= $order_subtotal ) {
					...
					if ( isset( $dgOptionsType ) && ! empty( $dgOptionsType ) && $dgOptionsType == 'HAZARDOUS_MATERIALS' ) {
					...
					if ( 'freight' === $request_type ) {
						...
						$request['RequestedShipment']['ShippingDocumentSpecification']['ShippingDocumentTypes'] = array();
						...
						foreach ( $request['RequestedShipment']['RequestedPackageLineItems'] as $key => $item ) {
							...
							if ( ! empty( $item['Dimensions'] ) ) {
								...
								$ph_max_dimension = max( array_map( array( $this, 'dimensions_in_inches' ), array( $item['Dimensions']['Length'], $item['Dimensions']['Width'], $item['Dimensions']['Height'] ) ) );

								if ( $ph_max_dimension >= 96 && $ph_max_dimension <= 144 ) {
									...
									break;
								} else if ( $ph_max_dimension > 144 ) {

									$request['RequestedShipment']['SpecialServicesRequested']['SpecialServiceTypes'][] = 'EXTREME_LENGTH';

									$request['RequestedShipment']['FreightShipmentDetail']['SpecialServicePayments'] = array(

										'SpecialService' => 'EXTREME_LENGTH',
										'PaymentType'    => 'SHIPPER',
									);
									break;
								}
						...
						// When Hazardous Materials In Package
						if ( $isHazardousMaterialsInPackage ) {

							// Adding SpecialServiceTypes DANGEROUS_GOODS in Shipment Level
							$request['RequestedShipment']['SpecialServicesRequested']['SpecialServiceTypes'][] = 'DANGEROUS_GOODS';

							// Adding Shipper Contact Number as Emergency Contact Number for Dangerous Goods
							$request['RequestedShipment']['FreightShipmentDetail']['HazardousMaterialsEmergencyContactNumber'] = $this->freight_shipper_phone_number;

							$request['RequestedShipment']['FreightShipmentDetail']['HazardousMaterialsOfferor'] = 'SENDER';
						}
					...
					$core_countries = array( 'US' );

					// Take origin address from shop address to support multiwarehouse scenario
					$origin_address       = $this->shop_address( $package );
					$this->origin_country = isset( $origin_address['Address']['CountryCode'] ) ? $origin_address['Address']['CountryCode'] : $this->origin_country;
					...
					if ( $this->origin_country !== $package['destination']['country'] || ! in_array( $this->origin_country, array( 'US' ) ) ) {
						...
						if ( $this->customs_duties_payer == 'SENDER' ) {
							...
							$request['RequestedShipment']['CustomsClearanceDetail']['DutiesPayment']['Payor']['ResponsibleParty'] = array(
								'AccountNumber' => apply_filters( 'ph_customs_third_party_acc_no', strtoupper( $this->third_party_acc_no ), $package ),
							);
						}
						...
						$request['RequestedShipment']['CustomsClearanceDetail']['CustomsValue'] = array(
							'Amount'   => round( $this->customtotal, 2 ),
							'Currency' => $custom_currency,
						);
						...
						if ( isset( $_GET['export_compliance'] ) ) {
							$complianceCode = $_GET['export_compliance'];
							$complianceCode = preg_replace( '/[^a-zA-Z0-9\s]/', '', $complianceCode );
						}

						$exportCompliance = $this->getExportComplianceDetails( $this->origin_country, $this->order->get_shipping_country(), $complianceCode );

						if ( ! empty( $exportCompliance ) ) {
							$request['RequestedShipment']['CustomsClearanceDetail']['ExportDetail'] = $exportCompliance;
						}
						...
						if ( $this->commercial_invoice && $this->is_international ) {
							...
							$this->csb_termsofsale = ( isset( $_GET['tos'] ) && ! empty( $_GET['tos'] ) ) ? $_GET['tos'] : $this->csb_termsofsale;

							if ( ! empty( $this->csb_termsofsale ) && $this->csb_termsofsale != 'none' ) {

										$request['RequestedShipment']['CustomsClearanceDetail']['CommercialInvoice']['TermsOfSale'] = $this->csb_termsofsale;
							}

							PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $this->order_id, '_ph_fedex_terms_of_sale', $this->csb_termsofsale );
							...
							if ( isset( $this->settings['custom_declaration_statement'] ) && ! empty( $this->settings['custom_declaration_statement'] ) ) {

								$request['RequestedShipment']['CustomsClearanceDetail']['CommercialInvoice']['DeclarationStatement'] = $this->settings['custom_declaration_statement'];
							}
						}
						...
						if ( $this->commercial_invoice_shipping && $this->is_international ) {
							...
							if ( $this->commercial_invoice_order_currency ) {
								...
								$shipping_currency = $this->wf_get_fedex_currency();
									...
									$request['RequestedShipment']['CustomsClearanceDetail']['CustomsValue']['Amount'] += $shipping_cost;
						}
	...
	/**
	 * Get export compliance details for shipment
	 *
	 * @param string $originCountry
	 * @param string $destinationCountry
	 * @param string $complianceCode
	 *
	 * @return array $exportCompliance
	 */
	public function getExportComplianceDetails( $originCountry, $destinationCountry, $complianceCode ) {

		$exportCompliance                         = array();
		$exportDetailRequiredShippingCountries    = array( 'CN', 'HK', 'BE', 'SA', 'FR' );
		$exportDetailNotRequiredShippingCountries = array( 'CA', 'US', 'PR', 'VI' );

		if ( ! empty( $complianceCode ) ) {

			PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $this->order_id, '_wf_fedex_export_compliance', $complianceCode );

			$b13Currency              = 'CAD';
			$fedexCurrency            = $this->wf_get_fedex_currency();
			$wcCurrencyConversionRate = get_option( 'woocommerce_multicurrency_rates' );
			$b13OrderCost             = $this->customtotal;

			if ( $fedexCurrency != $b13Currency && ! empty( $wcCurrencyConversionRate ) ) {

				$b13Currency_rate   = $wcCurrencyConversionRate[ $b13Currency ];
				$fedexCurrency_rate = $wcCurrencyConversionRate[ $fedexCurrency ];

				$conversion_rate = $b13Currency_rate / $fedexCurrency_rate;
				$b13OrderCost   *= $conversion_rate;

			}

			if ( ( $originCountry == 'CA' && $b13OrderCost >= 2000 && ! in_array( $destinationCountry, $exportDetailNotRequiredShippingCountries ) ) || ( $this->origin_country == 'US' && in_array( $this->order->get_shipping_country(), $exportDetailRequiredShippingCountries ) ) ) {

				$exportCompliance = array(
					'B13AFilingOption'          => 'FILED_ELECTRONICALLY',
					'ExportComplianceStatement' => $complianceCode,
				);
		...
		} elseif ( $originCountry == 'CA' && ! in_array( $destinationCountry, $exportDetailNotRequiredShippingCountries ) ) {
		...
		} elseif ( $this->origin_country == 'US' && in_array( $this->order->get_shipping_country(), $exportDetailRequiredShippingCountries ) ) {
	...
	public function wf_get_insurance_amount( $product ) {
		...
		$insured_price = current( $this->xa_get_custom_product_option_details( array( $product ), '_wf_fedex_custom_declared_value' ) );
	...
	public function wf_get_package_from_order( $order ) {
		...
		$package['destination']['first_name'] = $order->get_shipping_first_name();
		$package['destination']['last_name']  = $order->get_shipping_last_name();
		$package['destination']['company']    = $order->get_shipping_company();
		$package['destination']['address_1']  = $order->get_shipping_address_1();
		$package['destination']['address_2']  = $order->get_shipping_address_2();
		...
		$packages = apply_filters( 'wf_filter_label_packages', array( $package ), $this->ship_from_address, $order->get_id() ); // for multivendor
		return $packages;
	}
	...
	public function print_label( $order, $service_code, $order_id ) {
		...
		foreach ( $stored_packages as $key => $fedex_package ) {
			...
			foreach ( $grouped_packages as $group_name => $package ) {
				$this->print_label_processor( $package, $packages[ $key ], $order_id );
			}
	...
	private function get_return_request( $shipment_id, $order_id, $serviceCode ) {
		...
		$request = PH_WC_Fedex_Storage_Handler::ph_get_meta_data( $order_id, 'wf_woo_fedex_request_' . $shipment_id );
		...
		if ( isset( $request['RequestedShipment']['CustomsClearanceDetail'] ) ) {
			...
			// Modifying Commodities based on Package total weight, Because of Commodities mismatch for Commercial invoice
			if ( isset( $request['RequestedShipment']['CustomsClearanceDetail']['Commodities'] ) && count( $request['RequestedShipment']['CustomsClearanceDetail']['Commodities'] ) > 1 ) {

				$package_total_weight = $request['RequestedShipment']['TotalWeight']['Value'];

				foreach ( $request['RequestedShipment']['CustomsClearanceDetail']['Commodities'] as $key => $commodity ) {

					if ( $package_total_weight > 0 && $package_total_weight >= $commodity['Weight']['Value'] ) {

						$ph_qty_to_pass = 0;

						for ( $i = 0; $i < $commodity['Quantity']; $i++ ) {

							if ( $package_total_weight > 0 && $package_total_weight >= $commodity['Weight']['Value'] ) {

								$package_total_weight -= $commodity['Weight']['Value'];
								++$ph_qty_to_pass;
							}
						}
						$request['RequestedShipment']['CustomsClearanceDetail']['Commodities'][ $key ]['Quantity'] = $ph_qty_to_pass;
					} else {

						unset( $request['RequestedShipment']['CustomsClearanceDetail']['Commodities'][ $key ] );
					}
				}
			}
		}
	...
	public function print_return_label( $shipment_id, $order_id, $serviceCode ) {
		...
		if ( Ph_Fedex_Woocommerce_Shipping_Common::phIsNewRegistration() ) {
			// Check for active plugin license
			if ( ! Ph_Fedex_Woocommerce_Shipping_Common::phHasActiveLicense() ) {
				$this->admin_diagnostic_report( '------------------------------- RETURN SHIPMENT -------------------------------' );
				$this->admin_diagnostic_report( 'Please use a valid plugin license to continue using WooCommerce FedEx Shipping Plugin with Print Label' );
		...
		$this->process_result( $request, $this->get_result( $request, $order_id, $isNewAndActiveRegistration, $apiAccessDetails, 'return_ship' ) );

		$return_label = PH_WC_Fedex_Storage_Handler::ph_get_meta_data( $order_id, 'wf_woo_fedex_returnLabel_' . $shipment_id );

		$return_label_image_type = PH_WC_Fedex_Storage_Handler::ph_get_meta_data( $order_id, 'wf_woo_fedex_returnLabel_image_type_' . $shipment_id );

		if ( ! empty( $return_label ) ) {

			do_action( 'xa_fedex_label_generated_successfully', $shipment_id, $return_label, $order_id, $return_label_image_type, true );
		}
	}
	...
	public function void_shipment( $order_id, $shipment_id, $tracking_completedata ) {
		...
		try {
			...
			$wsdl_dir = plugin_dir_path( __DIR__ ) . 'fedex-wsdl/' . ( $this->production ? 'production' : 'test' ) . '/ShipService_v' . $this->ship_service_version . '.wsdl';

			if ( Ph_Fedex_Woocommerce_Shipping_Common::phIsNewRegistration() ) {
				if ( ! Ph_Fedex_Woocommerce_Shipping_Common::phHasActiveLicense() ) {
					$this->admin_diagnostic_report( '------------------------------- VOID SHIPMENT -------------------------------' );
					$this->admin_diagnostic_report( 'Please use a valid plugin license to continue using WooCommerce FedEx Shipping Plugin with Print Label' );
					...
					$proxyParams = Ph_Fedex_Woocommerce_Shipping_Common::phGetParamsForProxyCall( $apiAccessDetails, $request, 'cancel_ship' );
			...
			if ( $this->soap_method == 'nusoap' ) {
				...
				$result = Ph_Fedex_Woocommerce_Shipping_Common::phConvertArrayToObject( $result );
			} else {

				$result = $client->deleteShipment( $request );
			}
		} catch ( Exception $e ) {
			$this->debug( __( 'SoapFault while void_shipment.', 'ph-fedex-woocommerce-shipping' ) );
		}

		$this->debug( 'FedEx REQUEST: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $request, true ) . '</pre>' );
		$this->debug( 'FedEx RESPONSE: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $result, true ) . '</pre>' );
		...
		if ( is_object( $result ) && $result->HighestSeverity != 'FAILURE' && $result->HighestSeverity != 'ERROR' ) {
			...
			PH_WC_Fedex_Storage_Handler::ph_add_and_save_meta_data( $order_id, 'wf_woo_fedex_shipment_void', $shipment_id );
			...
			PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $order_id, 'wf_woo_fedex_shipment_void_errormessage', $shipment_void_errormessage );
			PH_WC_Fedex_Storage_Handler::ph_add_and_save_meta_data( $order_id, 'ph_woo_fedex_shipment_client_reset', $shipment_id );
		}
	}

	public function print_label_processor( $fedex_packages, $package, $order_id ) {

		$this->master_tracking_id = '';

		// Debugging
		$this->debug( __( 'FEDEX debug mode is on - to hide these messages, turn debug mode off in the settings.', 'ph-fedex-woocommerce-shipping' ) );

		// See if address is residential
		$this->residential_address_validation( $package );

		$request_type = '';
		if ( ! empty( $this->smartpost_hub ) && $package['destination']['country'] == 'US' && ( $this->service_code == 'SMART_POST' || current( $fedex_packages )['service'] == 'SMART_POST' ) ) {
			$request_type = 'smartpost';
		} elseif ( in_array( $this->service_code, array( 'FEDEX_FREIGHT_ECONOMY', 'FEDEX_FREIGHT_PRIORITY' ) ) !== false ) {
		...
		if ( $this->validate_package( $fedex_packages ) ) {
			...
			if ( $fedex_requests ) {
				$this->run_package_request( $fedex_requests, $order_id );
			}
			$packages_to_quote_count = sizeof( $fedex_requests );
		}

		PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $this->order_id, 'wf_woo_fedex_shipmentErrorMessage', $this->shipmentErrorMessage );
	}
	...
	public function manual_packages( $packages ) {
		...
		PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $this->order_id, '_wf_fedex_stored_packages', $packages );

		return $packages;
	}

	public function run_package_request( $requests, $order_id ) {
		...
		foreach ( $requests as $key => $request ) {
			...
			if ( $this->commercial_invoice && $this->is_international ) {
				...
				if ( $etd_label ) {
							...
							$request['RequestedShipment']['SpecialServicesRequested']['EtdDetail']['RequestedDocumentCopies']   = array();
			...
			$this->process_result( $request, $this->get_result( $request, $order_id, $isNewAndActiveRegistration, $apiAccessDetails, 'ship' ) );
		}
	...
	public function get_result( $request, $order_id, $isNewAndActiveRegistration = false, $apiAccessDetails = array(), $type = '' ) {

		// SmartPost does not support Multi-piece shipments.
		if ( ! empty( $this->pre_service ) && ( $this->pre_service !== $request['RequestedShipment']['ServiceType'] || $request['RequestedShipment']['ServiceType'] == 'SMART_POST' ) ) {
		...
		try {
			...
			if ( $isNewAndActiveRegistration ) {
				...
				$type = ! empty( $type ) ? $type : 'ship';

				$proxyParams = Ph_Fedex_Woocommerce_Shipping_Common::phGetParamsForProxyCall( $apiAccessDetails, $request, $type );
			...
			$this->debug( __( 'SoapFault while run_package_request.', 'ph-fedex-woocommerce-shipping' ) );
		...
		if ( WF_FEDEX_ADV_DEBUG_MODE == 'on' ) { // Test mode is only for development purpose.
			...
			if ( $this->debug ) {
				...
				$this->admin_diagnostic_report( "------------------------------- Fedex Create Shipment Request - $order_id -------------------------------" );
				$this->admin_diagnostic_report( htmlspecialchars( $xml_request ) );
				$this->admin_diagnostic_report( "------------------------------- Fedex Create Shipment Response - $order_id -------------------------------" );
				$this->admin_diagnostic_report( htmlspecialchars( $xml_response ) );
			}
		}
		return $result;
	...
	private function process_result( $request, $result = null ) {
		if ( ! $result ) {
			return false;
		}
		...
		if ( $result->HighestSeverity != 'FAILURE' && $result->HighestSeverity != 'ERROR' && ! empty( $result->CompletedShipmentDetail ) ) {
			...
			if ( ! empty( $shippingLabel ) && property_exists( $result->CompletedShipmentDetail, 'AssociatedShipments' ) && property_exists( $result->CompletedShipmentDetail->AssociatedShipments, 'Label' ) ) {
				...
				if ( ! empty( $result->CompletedShipmentDetail->AssociatedShipments->TrackingId ) ) {
					...
					$order_meta_handler->ph_add_meta_data( '_ph_woo_fedex_additional_tracking_number_' . $shipmentId, $associated_documents_tracking_id );
				}
			...
			if ( ! empty( $this->shipmentId ) && property_exists( $result->CompletedShipmentDetail->CompletedPackageDetails->Label, 'ShippingDocumentDisposition' ) && $result->CompletedShipmentDetail->CompletedPackageDetails->Label->ShippingDocumentDisposition == 'RETURNED' ) {
				...
				if ( ! empty( $addittional_label ) ) {
					...
					$order_meta_handler->ph_add_meta_data( 'ph_fedex_additional_return_label_' . $this->shipmentId, $addittional_label, true );

					if ( ! empty( $addittional_label_type ) ) {
						$order_meta_handler->ph_add_meta_data( 'ph_fedex_additional_return_label_image_type_' . $this->shipmentId, $addittional_label_type, true );
					}
				}

				$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_returnShipmetId', $shipmentId, true );
				$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_returnLabel_' . $this->shipmentId, $shippingLabel, true );

				if ( ! empty( $shippinglabel_type ) ) {
					$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_returnLabel_image_type_' . $this->shipmentId, $shippinglabel_type, true );
				}
			...
			if ( ! empty( $shipmentId ) && ! empty( $shippingLabel ) ) {
				...
				$shipmentIds = PH_WC_Fedex_Storage_Handler::ph_get_meta_data( $this->order_id, 'ph_woo_fedex_shipmentIds' );
				...
				$order_meta_handler->ph_delete_meta_data( 'ph_woo_fedex_shipmentIds' );
				$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_shipmentId', $shipmentId );

				// Some Customers Site wont allow adding duplicate Meta Keys in DB, Adding new meta key with custom build Shipment Id Array
				$order_meta_handler->ph_add_meta_data( 'ph_woo_fedex_shipmentIds', $shipmentIds );
				$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_shippingLabel_' . $shipmentId, $shippingLabel, true );
				$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_packageDetails_' . $shipmentId, $this->wf_get_parcel_details( $request ), true );
				$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_request_' . $shipmentId, $request, true );

				if ( ! empty( $shippinglabel_type ) ) {
					$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_shippingLabel_image_type_' . $shipmentId, $shippinglabel_type, true );
				}

				if ( isset( $tracking_completedata ) ) {
					$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_tracking_full_details_' . $shipmentId, $tracking_completedata, true );
				}

				if ( ! empty( $request['RequestedShipment']['ServiceType'] ) ) {
					$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_service_code' . $shipmentId, $request['RequestedShipment']['ServiceType'], true );
				}

				if ( ! empty( $usps_shipmentId ) ) {
					$order_meta_handler->ph_add_meta_data( 'wf_woo_fedex_usps_trackingid_' . $shipmentId, $usps_shipmentId, true );
				}

				if ( $this->add_trackingpin_shipmentid == 'yes' && ! empty( $shipmentId ) ) {
					// $this->order->add_order_note( sprintf( __( 'Fedex Tracking-pin #: %s.', 'ph-fedex-woocommerce-shipping' ), $shipmentId) , true);
					$this->tracking_ids = $this->tracking_ids . $shipmentId . ',';
				}

				if ( $this->add_trackingpin_shipmentid == 'yes' && ! empty( $usps_shipmentId ) ) {
					// $this->order->add_order_note( sprintf( __( 'Fedex Smart Post USPS Tracking-pin #: %s.', 'ph-fedex-woocommerce-shipping' ), $usps_shipmentId) , true);
				}

				if ( ! empty( $addittional_label ) ) {

					$order_meta_handler->ph_add_meta_data( 'wf_fedex_additional_label_' . $shipmentId, $addittional_label, true );
					if ( ! empty( $addittional_label_type ) ) {
						$order_meta_handler->ph_add_meta_data( 'wf_fedex_additional_label_image_type_' . $shipmentId, $addittional_label_type, true );
					}
				}
			}
	...
	public function result_notifications( $notes, $error_message = '', $label = false ) {
		...
		$authenitication_failed = '<br/>' . __( "It appears your FedEx Production Account hasn't been validated yet. No worries, we've got you covered!", 'ph-fedex-woocommerce-shipping' ) . '<br/>
									' . __( "Our plugin supports direct FedEx Account Registration, so you can quickly resolve this issue. Here's how:", 'ph-fedex-woocommerce-shipping' ) . "<br/>
									<ul style='list-style-type: circle; padding-left: 1%'>
										<li>" . __( 'Click', 'ph-fedex-woocommerce-shipping' ) . " <a href='https://www.pluginhive.com/knowledge-base/setting-woocommerce-fedex-shipping-plugin/#donot_have_fedex_meter_number' target='_blank'> " . __( 'this link', 'ph-fedex-woocommerce-shipping' ) . '</a> ' . __( 'to view the registration process.', 'ph-fedex-woocommerce-shipping' ) . '</li>
										<li>' . __( 'Follow the steps to complete the registration process.', 'ph-fedex-woocommerce-shipping' ) . '</li>
									</ul>
									' . __( "Once you're registered, you should be able to access all features without any problems.", 'ph-fedex-woocommerce-shipping' ) . '<br/><br/>
									' . __( 'Still need help?', 'ph-fedex-woocommerce-shipping' ) . '<br/>
									' . __( "Don't hesitate to contact", 'ph-fedex-woocommerce-shipping' ) . " <a href='https://www.pluginhive.com/support/' target='_blank'>" . __( 'PluginHive support', 'ph-fedex-woocommerce-shipping' ) . ' </a>' . __( 'for further assistance.', 'ph-fedex-woocommerce-shipping' ) . '<br/>';
	...
	private function shop_address( $package = '' ) {
		...
		$request = array(
			...
			'Address' => array(
				...
				'CountryCode'         => array_key_exists(strtoupper( $from_address['country'] ), Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper) ? Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper[strtoupper( $from_address['country'] )] : strtoupper( $from_address['country'] ),
	...
	private function order_address( $package ) {
		...
		$addr = array(
			...
			'Address' => array(
				...
				'StateOrProvinceCode' => strlen( $package['destination']['state'] ) == 2 ? strtoupper( $package['destination']['state'] ) : '',
				'CountryCode'         => array_key_exists($package['destination']['country'], Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper) ? Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper[$package['destination']['country']] : $package['destination']['country'],
			),
		);
		return $addr;
	}
	...
	private function manual_dimensions( $package ) {
		...
		if ( empty( $_GET['weight'] ) ) {
			$this->debug( sprintf( __( '<br> Package weight is missing. Aborting.', 'ph-fedex-woocommerce-shipping' ) ), 'error' );
			return;
		}
		...
		foreach ( $fedex_packages as $key => $parcel ) {
			if ( $parcel['packed_products'] ) {
				foreach ( $parcel['packed_products'] as $pid => $product ) {
					...
					$wf_hs_code = $this->xa_get_custom_product_option_details( array( $parcel['packed_products'][ $pid ] ), '_wf_hs_code' );
					...
					if ( ! empty( $wf_hs_code ) ) {
						...
						$commodoties[ $commodity_id ]['HarmonizedCode'] = current( $wf_hs_code );
					} elseif ( ! empty( $this->global_hs_code ) ) {
...
*/
	...
	public function request_pickup( $order_ids = array() ) {
		...
		$domestic_ground_associate_order_ids  = array();
		$domestic_freight_associate_order_ids = array();
		$domestic_express_associate_order_ids = array();
		...
		$int_ground_associate_order_ids  = array();
		$int_freight_associate_order_ids = array();
		$int_express_associate_order_ids = array();

		$int_ground_order_id      = 0;
		$int_ground_package_count = 0;
		$int_ground_weight        = 0;
		...
		$pickupStartTimeOptions = array();
		$pickupCloseTimeOptions = array();

		// Pickup ready time must contain a time between 08:00am and 06:00pm
		foreach ( range( 8, 18, 0.5 ) as $pickupStartTime ) {
			$pickupStartTimeOptions[ (string) $pickupStartTime ] = date( 'H:i', strtotime( date( 'Y-m-d' ) ) + 3600 * $pickupStartTime );
		}

		// Pickup ready time must contain a time between 08:00am and 06:00pm
		foreach ( range( 8.5, 24, 0.5 ) as $pickupCloseTime ) {
			$pickupCloseTimeOptions[ (string) $pickupCloseTime ] = date( 'H:i', strtotime( date( 'Y-m-d' ) ) + 3600 * $pickup_close_time );
		}

		// Get start time and close time in H:i format to compare
		$pickupStartTime = $pickupStartTimeOptions[ (string) $pickup_start_time ];
		$pickupCloseTime = $pickupCloseTimeOptions[ (string) $pickup_close_time ];

		// Check Current Time exceeds Store Cut-off Time, if yes request on next day
		if ( ! empty( $pickupCloseTime ) && $pickupCloseTime != '24:00' ) {

			$this->current_wp_time_hour_minute = current_time( 'H:i' );

			if ( strtotime( $this->current_wp_time_hour_minute ) > strtotime( $pickupCloseTime ) || strtotime( $this->current_wp_time_hour_minute ) > strtotime( $pickupStartTime ) ) {
			...
			foreach ( $packages as $package ) {
				...
				if ( is_array( $shipmentIds ) && ! empty( $shipmentIds ) ) {
					foreach ( $shipmentIds as $shipmentId ) {
						...
						$package_service[ $shipmentId ]    = array();
						$package_service[ $shipmentId ][0] = $order_id;
						$package_service[ $shipmentId ][1] = PH_WC_Fedex_Storage_Handler::ph_get_meta_data( $order_id, 'wf_woo_fedex_service_code' . $shipmentId );
						$package_service[ $shipmentId ][2] = PH_WC_Fedex_Storage_Handler::ph_get_meta_data( $order_id, 'wf_woo_fedex_packageDetails_' . $shipmentId );
					...
					if ( ! empty( $package_service ) ) {
						foreach ( $package_service as $track_id => $service ) {
							...
							if ( $service[1] == 'FEDEX_GROUND' || $service[1] == 'GROUND_HOME_DELIVERY' ) {
								...
								$domestic_ground_associate_order_ids[] = $order_id;
							...
							} elseif ( in_array( $service[1], $freight_array ) ) {

								$domestic_freight_associate_order_ids[] = $order_id;
								...
								$domestic_express_associate_order_ids[] = $order_id;
					...
					if ( ! empty( $package_service ) ) {
						foreach ( $package_service as $track_id => $service ) {
							...
							if ( $service[1] == 'FEDEX_GROUND' || $service[1] == 'GROUND_HOME_DELIVERY' ) {
								...
								$int_ground_associate_order_ids[] = $order_id;

								$int_ground_order_id       = $order_id;
								$int_ground_package_count += 1;
								$int_ground_weight        += trim( preg_replace( '/[A-Za-z ]/', '', $service[2]['Weight'] ) );

							} elseif ( $service[1] == 'INTERNATIONAL_ECONOMY_FREIGHT' || $service[1] == 'INTERNATIONAL_PRIORITY_FREIGHT' ) {

								$int_freight_associate_order_ids[] = $order_id;

								$int_freight_order_id       = $order_id;
								$int_freight_service        = $service[1];
								$int_freight_package_count += 1;
								$int_freight_weight        += trim( preg_replace( '/[A-Za-z ]/', '', $service[2]['Weight'] ) );

							} else {

								$int_express_associate_order_ids[] = $order_id;
		...
		if ( ! empty( $domestic_package ) && is_array( $domestic_package ) && ! empty( $domestic_order_id ) ) {
			...
			$origin_address['Address']['StateOrProvinceCode'] = $this->ph_state_mapper_for_pickup( $origin_address['Address']['StateOrProvinceCode'], $origin_address['Address']['CountryCode'] );
			...
			$response[ $domestic_order_id ]            = array();
			$response[ $domestic_order_id ]['Express'] = array();
			$response[ $domestic_order_id ]['Ground']  = array();
			$response[ $domestic_order_id ]['Freight'] = array();
			...
			if ( ! empty( $domestic_express_weight ) && ! empty( $domestic_express_package_count ) ) {
				...
				$response[ $domestic_express_order_id ]['Express']['Response'][]      = $this->run_pickup_request( $express_request );
				$response[ $domestic_express_order_id ]['Express']['All_Order_Ids']   = $domestic_express_associate_order_ids;
				$response[ $domestic_express_order_id ]['Express']['PickupTimestamp'] = $pickup_ready_timestamp;
			}
			...
			if ( ! empty( $domestic_ground_weight ) && ! empty( $domestic_ground_package_count ) ) {
				...
				$response[ $domestic_ground_order_id ]['Ground']['Response'][]      = $this->run_pickup_request( $ground_request );
				$response[ $domestic_ground_order_id ]['Ground']['All_Order_Ids']   = $domestic_ground_associate_order_ids;
				$response[ $domestic_ground_order_id ]['Ground']['PickupTimestamp'] = $ground_ready_timestamp;
			}
			...
			if ( ! empty( $domestic_freight_weight ) && ! empty( $domestic_freight_package_count ) ) {
				...
				$response[ $domestic_freight_order_id ]['Freight']['Response'][]      = $this->run_pickup_request( $freight_request );
				$response[ $domestic_freight_order_id ]['Freight']['All_Order_Ids']   = $domestic_freight_associate_order_ids;
				$response[ $domestic_express_order_id ]['Freight']['PickupTimestamp'] = $pickup_ready_timestamp;
			}
		}
		...
		if ( ! empty( $international_package ) && is_array( $international_package ) && ! empty( $int_order_id ) ) {
			...
			$response[ $int_order_id ]               = array();
			$response[ $int_order_id ]['IntGround']  = array();
			$response[ $int_order_id ]['IntExpress'] = array();
			$response[ $int_order_id ]['IntFreight'] = array();

			if ( ! empty( $int_ground_weight ) && ! empty( $int_ground_package_count ) ) {

				$request_int_ground['OriginDetail'] = array(
					'UseAccountAddress' => 0,
					'PickupLocation'    => $pickup_address,
					'ReadyTimestamp'    => date( 'Y-m-d\TH:i:s', $pickup_ready_timestamp ),
					'CompanyCloseTime'  => date( 'H:i:s', $pickup_close_timestamp ),
				...
				$request_int_ground['CarrierCode']  = 'FDXG';
				$request_int_ground['PackageCount'] = $int_ground_package_count;
				$request_int_ground['TotalWeight']  = array(
					'Units' => $this->labelapi_weight_unit,
					'Value' => $int_ground_weight,
				);

				$int_ground_request = array_merge( $request, $request_int_ground );

				$response[ $int_ground_order_id ]['IntGround']['Response'][]      = $this->run_pickup_request( $int_ground_request );
				$response[ $int_ground_order_id ]['IntGround']['All_Order_Ids']   = $int_ground_associate_order_ids;
				$response[ $int_ground_order_id ]['IntGround']['PickupTimestamp'] = $pickup_ready_timestamp;
			}
			...
			if ( ! empty( $int_express_weight ) && ! empty( $int_express_package_count ) ) {
				...
				$response[ $int_express_order_id ]['IntExpress']['Response'][]      = $this->run_pickup_request( $int_express_request );
				$response[ $int_express_order_id ]['IntExpress']['All_Order_Ids']   = $int_express_associate_order_ids;
				$response[ $int_express_order_id ]['IntExpress']['PickupTimestamp'] = $pickup_ready_timestamp;
			}
			...
			if ( ! empty( $int_freight_weight ) && ! empty( $int_freight_package_count ) ) {
				...
				$response[ $int_freight_order_id ]['IntFreight']['Response'][]      = $this->run_pickup_request( $int_freight_request );
				$response[ $int_freight_order_id ]['IntFreight']['All_Order_Ids']   = $int_freight_associate_order_ids;
				$response[ $int_freight_order_id ]['IntFreight']['PickupTimestamp'] = $pickup_ready_timestamp;
			}
		}

		return $this->process_pickup_response(
			$response,
			array(
				'OrderId'       => $master_order_id,
				'ScheduledDate' => date( 'Y-m-d', $pickup_ready_timestamp ),
			)
		);
	}
	
	/**
	 * Mapping State Code for Pickup
	 * 
	 * @param $ph_state_code
	 * @param $ph_country_code
	 * @return $ph_state_code
	 */
	public function ph_state_mapper_for_pickup( $ph_state_code, $ph_country_code ) {

		$ph_country_state_codes = array(
			"MX" => array(
				"AGS" 	=> "AG",
				"BC" 	=> "BC",
				"BCS" 	=> "BS",
		...
		$ph_state_code = isset($ph_country_state_codes[$ph_country_code]) && isset($ph_country_state_codes[$ph_country_code][$ph_state_code])? $ph_country_state_codes[$ph_country_code][$ph_state_code] : $ph_state_code;
	...
	public function run_pickup_request( $request ) {
		try {
			...
			$wsdl_dir = plugin_dir_path( __DIR__ ) . 'fedex-wsdl/' . ( $this->production ? 'production' : 'test' ) . '/PickupService_v' . $this->pickup_service_version . '.wsdl';

			// Check if new registration method
			if ( Ph_Fedex_Woocommerce_Shipping_Common::phIsNewRegistration() ) {
				// Check for active plugin license
				if ( ! Ph_Fedex_Woocommerce_Shipping_Common::phHasActiveLicense() ) {
					wf_admin_notice::add_notice( 'Please use a valid plugin license to continue using WooCommerce FedEx Shipping Plugin with Print Label', 'notice' );
					return '';
				} else {
					...
					$proxyParams = Ph_Fedex_Woocommerce_Shipping_Common::phGetParamsForProxyCall( $apiAccessDetails, $request, 'pickup' );
		...
		foreach ( $response as $order_id => $result ) {
			...
			if ( ! empty( $result ) ) {
				foreach ( $result as $service => $service_data ) {
					...
					if ( ! empty( $service_data['Response'] ) ) {
						...
						foreach ( $service_data['Response'] as $result_data ) {

							$return[ $order_id ][ $service ]['for_orders'] = $service_data['All_Order_Ids'];
							$return[ $order_id ][ $service ]['PickupTime'] = date( 'Y-m-d', $service_data['PickupTimestamp'] );
							...
							} elseif ( ! isset( $result_data->Notifications->Code ) ) {

										$return[ $order_id ][ $service ]['error']   = 1;
										$return[ $order_id ][ $service ]['message'] = 'Unexpected error';
							} elseif ( $result_data->Notifications->Code != '0000' ) {
								$return[ $order_id ][ $service ]['error']   = 1;
	...
	public function pickup_cancel( $order, $order_id, $pickup_details = array() ) {
		...
		$use_pickup_address = ( $bool = $this->settings['use_pickup_address'] ) && $bool == 'yes' ? true : false;
		$order_data         = wc_get_order( $this->order_id );
		...
		try {
			...
			if ( Ph_Fedex_Woocommerce_Shipping_Common::phIsNewRegistration() ) {
				...
				if ( ! Ph_Fedex_Woocommerce_Shipping_Common::phHasActiveLicense() ) {
					...
					$proxyParams = Ph_Fedex_Woocommerce_Shipping_Common::phGetParamsForProxyCall( $apiAccessDetails, $request, 'cancel_pickup' );
	...
	public function process_pickup_cancel( $response, $info = array() ) {
		...
		foreach ( $response as $response_data ) {
			...
			} elseif ( ! isset( $response_data->Notifications->Code ) ) {
					$return['error']   = 1;
					$return['message'] = $response_data['message'];
			} elseif ( $response_data->Notifications->Code != '0000' ) {
				$return['error']   = 1;
	...
	private function xa_get_volumatric_products_weight( $values ) {
		...
		$weight = $this->round_up( ( ! empty( $values->get_weight() ) ? $values->get_weight() : 0 ), 2 );

		if ( ! empty( $volumetric_weight ) ) {
			$volumetric_weight = $this->round_up( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_weight( $volumetric_weight, get_option( 'woocommerce_weight_unit' ), 'kg' ), 2 );
