...
class wf_fedex_woocommerce_shipping_method extends WC_Shipping_Method {
	 ...
	 * Versions Variables
	 */
	public $rateservice_version, $addressvalidationservice_version;
	/**
	 * Convert Currency
	 */
	public $convert_currency_to_base;
	/**
	 * Admin Details Variables
	 */
	public $origin, $account_number, $meter_number, $smartpost_hub;
	/**
	 * General Settings Variables
	 */
	public $indicia, $ship_from_address, $api_key, $api_pass, $production, $debug, $silent_debug, $soap_method;

	public $insure_contents, $min_order_amount_for_insurance, $residential, $savedMetrics, $dimension_unit, $weight_unit, $skip_products, $working_days, $labelapi_dimension_unit, $labelapi_weight_unit;
	/**
	 * Rates and Services Settings Variables
	 */
	public $delivery_time, $delivery_time_details, $cut_off_time, $current_wp_time_hour_minute, $request_type, $conversion_rate, $min_shipping_cost, $max_shipping_cost, $custom_services, $offer_rates, $fedex_one_rate, $fedex_one_rate_package_ids, $fedex_cod_rate, $shipping_charge, $saturday_delivery, $min_amount, $ship_time_adjustment, $fedex_currency, $fedex_conversion_rate, $fallback_rate;
	/**
	 * Label Settings Variables
	 */
	public $image_type;
	/**
	 * International Form Settings Variables
	 */
	public $global_hs_code;
	/**
	 * Special Services
	 */
	public $is_dry_ice_enabled, $dropoff_type, $signature_option, $customs_duties_payer;
	/**
	 * Packing Settings Variables
	 */
	public $packaging_type, $packing_method, $boxes, $box_max_weight, $weight_pack_process, $enable_speciality_box, $mode, $stack_to_volume, $speciality_boxes;
	/**
	 * Pickup
	 */
	public $saturday_pickup;
	/**
	 * Freight Variables
	 */
	public $freight_enabled, $freight_class, $freight_number, $freight_bill_street, $freight_billing_street_2, $freight_billing_city, $freight_billing_state, $freight_billing_postcode, $freight_billing_country, $frt_shipper_street, $freight_shipper_street_2, $freight_shipper_city, $freight_shipper_residential;
	/**
	 * WC Store Currency
	 */
	public $wc_store_currency;
	/**
	 * WC Store Currency
	 */
	public $satday_rates;
	...
	/**
	 * Alt Return Variables
	 */
	public $alt_return_country, $alt_return_state;
	/**
	 * International Variables
	 */
	public $internationalPakcagingFromUs, $internationalPakcagingFromNonUs;
	/**
	 * Domestic Variables
	 */
	public $domesticPackagingForUS, $domesticPackagingForNonUS, $destinationCountryCode;
	/**
	 * Current Package Items and Quantity
	 */
	 ...
	 * Unit Variables
	 */
	public $dimensionUnit, $weightUnit;
	...
	public $noPostcodeCountryArray = array('AE','AF','AG','AI','AL','AN','AO','AW','BB','BF','BH','BI','BJ','BM','BO','BS','BT','BW','BZ','CD','CF','CG','CI','CK','CL','CM','CR','CV','DJ','DM','DO','EC','EG','ER','ET','FJ','FK','GA','GD','GH','GI','GM','GN','GQ','GT','GW','GY','HK','HN','HT','IE','IQ','IR','JM','JO','KE','KH','KI','KM','KN','KP','KW','KY','LA','LB','LC','LR','LS','LY','ML','MM','MO','MR','MS','MT','MU','MW','MZ','NA','NE','NG','NI','NP','NR','NU','OM','PA','PE','PF','PY','QA','RW','SB','SC','SD','SL','SN','SO','SR','SS','ST','SV','SY','TC','TD','TG','TL','TO','TT','TV','TZ','UG','VC','VE','VG','VU','WS','XA','XB','XC','XE','XL','XM','XN','XS','YE','ZM','ZW');
	...
	public function __construct() {
		...
		$this->method_title					= __( 'FedEx', 'ph-fedex-woocommerce-shipping' );
		$this->method_description 			= __( 'WooCommerce FedEx Shipping Plugin with Print Label by PluginHive', 'ph-fedex-woocommerce-shipping' );
	...
	private function init() {
		...
		$this->savedMetrics				= $this->get_option( 'dimension_weight_unit' );
		...
		$this->getServicesBasedOnOrigin();

		$this->internationalPakcagingFromUs = [
		...
		$this->internationalPakcagingFromNonUs = [
			'FEDEX_10KG_BOX' => 'FEDEX_10KG_BOX',
			'FEDEX_25KG_BOX' => 'FEDEX_25KG_BOX',
		...
		$this->domesticPackagingForNonUS = [
			'FEDEX_SMALL_BOX' => 'FEDEX_BOX',
			'FEDEX_MEDIUM_BOX' => 'FEDEX_BOX',
			'FEDEX_LARGE_BOX' => 'FEDEX_BOX',
			'FEDEX_EXTRA_LARGE_BOX' => 'FEDEX_BOX',
		...
		if( isset( $_GET['reset_boxes'] ) ) {
			$this->ph_fedex_reset_boxes();
		}
	}
	 ...
	 * Reset boxes
	 */
	public function ph_fedex_reset_boxes() {
		
		$this->update_option( 'boxes', [] );
		wp_redirect( admin_url( 'admin.php?page=' . wf_get_settings_url() . '&tab=shipping&section=wf_fedex_woocommerce_shipping') );
	 ...
	 * @return array $boxes
	 */
	public function ph_convert_box_measurement_units( $boxes ) {
		...
		foreach( $boxes as $key => $box ) {
			...
			$boxes[$key]['length']			= round( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_dimension( $box['length'], $dimToUnit, $dimFromUnit ), 2 );
			$boxes[$key]['width']			= round( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_dimension( $box['width'], $dimToUnit, $dimFromUnit ), 2 );
			$boxes[$key]['height']			= round( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_dimension( $box['height'], $dimToUnit, $dimFromUnit ), 2 );
			$boxes[$key]['inner_length']	= round( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_dimension( $box['inner_length'], $dimToUnit, $dimFromUnit), 2 );
			$boxes[$key]['inner_width']		= round( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_dimension( $box['inner_width'], $dimToUnit, $dimFromUnit ), 2 );
			$boxes[$key]['inner_height']	= round( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_dimension( $box['inner_height'], $dimToUnit, $dimFromUnit ), 2 );
			$boxes[$key]['box_weight']		= round( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_weight( $box['box_weight'], $weightToUnit, $weightFromUnit ), 2 );
			$boxes[$key]['max_weight']		= round( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_weight( $box['max_weight'], $weightToUnit, $weightFromUnit ), 2 );
	...
	/**
	 * Get service list based on the origin country
	 */
	public function getServicesBasedOnOrigin() {
		...
		// Currently old service 'INTERNATIONAL_PRIORITY' is supported by country PR, once FedEx rolls out new service for PR this should be removed.
		if( $this->origin_country == 'PR' ) {
			$this->services['INTERNATIONAL_PRIORITY'] = 'FedEx International Priority®';
		}
	}
	...
	public function custom_checkout_field_update_order_meta($order_id)
	{
		...
		if (!empty($_POST['xa_fedex_lift_gate_for_delivery'])) {
			PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, 'xa_fedex_lift_gate_for_delivery', esc_attr($_POST['xa_fedex_lift_gate_for_delivery']));
		}

		// Update Inside Delivery to the order meta
		if (!empty($_POST['xa_fedex_inside_delivery'])) {
			PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, 'xa_fedex_inside_delivery', esc_attr($_POST['xa_fedex_inside_delivery']));
		}

		if (!is_admin()) {

			$suggested_adress 	= WC()->session->get('ph_address_validation');
			$meta_key			= PH_WC_Fedex_Storage_Handler::ph_get_meta_data($order_id, 'ph_fedex_suggested_address');

			if (!empty($suggested_adress) && is_array($suggested_adress) && empty($meta_key)) {

				PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, 'ph_fedex_suggested_address', $suggested_adress);
				...
				$order->add_order_note(__('FedEx Address Suggestion: ' . $address, 'ph-fedex-woocommerce-shipping'));
	...
	public function xa_add_fields_to_checkout( $fields ){
		...
		if ($this->settings['freight_enabled'] == 'no') {
			return $fields;
		}
		...
		if( ! empty($this->settings['lift_gate_for_delivery']) && $this->settings['lift_gate_for_delivery'] == 'yes' ) {
			$fields['billing']['xa_fedex_lift_gate_for_delivery'] = array(
				'label'			=> __('Lift gate required on delivery', 'ph-fedex-woocommerce-shipping'),
				'required'		=> false,
				'clear'			=> false,
				'type'			=> 'checkbox',
		...
		if( ! empty($this->settings['inside_delivery']) && $this->settings['inside_delivery'] == 'yes' ) {
			$fields['billing']['xa_fedex_inside_delivery'] = array(
				'label'			=> __('Inside delivery', 'ph-fedex-woocommerce-shipping'),
				'required'		=> false,
				'clear'			=> false,
				'type'			=> 'checkbox',
				'class'			=> array ('address-field', 'update_totals_on_change' ),
    ...
    **/
	...
	private function environment_check() {
		if ( ! in_array( get_woocommerce_currency(), array( 'USD' ) )) {
			echo '<div class="notice">
				<p>' . __( 'FedEx API returns the rates in USD. Please enable Rates in base currency option in the plugin. Conversion happens only if FedEx API provide the exchange rates.', 'ph-fedex-woocommerce-shipping' ) . '</p>
			</div>'; 
		} 
			
		if ( ! $this->origin && $this->enabled == 'yes' ) {
			echo '<div class="error">
				<p>' . __( 'FedEx is enabled, but the origin postcode has not been set.', 'ph-fedex-woocommerce-shipping' ) . '</p>
			</div>';
		}
	}
	...
	public function admin_options() {
		...
		echo '<h3>' . ( ! empty( $this->method_title ) ? $this->method_title : __( 'Settings', 'ph-fedex-woocommerce-shipping' ) ) . '</h3>';
	...
	public function generate_fedex_email_format_html() {
		global $woocommerce;
		ob_start();
		?>
		<tr valign="top" class="fedex_label_tab ph_email_content">
			<th scope="row" class="titledesc">
				<label for="email_format"><?php _e( 'Email Format', 'ph-fedex-woocommerce-shipping' ); ?></label>
				<span class="woocommerce-help-tip" data-tip="<?php echo _e( 'Define your own email html here. Use the place holder tag [DOWNLOAD LINK] to get the label dowload link. Supported tag holders - [DOWNLOAD LINK], [ORDER NO], [ORDER AMOUNT], [PRODUCTS ID], [PRODUCTS SKU], [PRODUCTS NAME], [CUSTOMER EMAIL], [CUSTOMER NAME], [ADDITIONAL LABELS], [PRODUCT_INFO] - Print Product details as table.', 'ph-fedex-woocommerce-shipping' ) ?>"><img src="<?php echo site_url("/wp-content/plugins/woocommerce/assets/images/help.png") ?>" height="16" width="16" /></span>
			</th>
			<td class="forminp">
				<textarea rows="10" cols="20" name="ph_fedex_email_format" id="ph_fedex_email_format"  
	...
	<div>Please Download the label</div>
	<a href='[DOWNLOAD LINK]' ><button>Download the label here</button> </a>
</body></html>"><?php echo isset( $this->settings['email_content'] ) ? $this->settings['email_content'] : ''; ?></textarea>
			</td>
		</tr>
		<?php
		return ob_get_clean();
	 ...
	 * @return void
	 */
	function generate_fedex_registration_banner_html()
	{

		ob_start();

		if (!Ph_Fedex_Woocommerce_Shipping_Common::phIsNewRegistration() && empty($this->meter_number)) {
		?>

			<div class="ph_registration_banner">

				<div id='ph_fedex_close_banner'>[X]</div>

				<div class="ph_fedex_box_1">

					<img src="<?php echo plugins_url('fedex-woocommerce-shipping') . '/resources/images/PH_Logo.png'; ?>">

					<h4><?php _e("WooCommerce FedEx Shipping Plugin with Print Label", "ph-fedex-woocommerce-shipping"); ?></h4>

				</div>

				<div class="ph_fedex_box_2">

					<ul>
						<li style='color:red;'><strong><?php _e("Don't have FedEx Access Keys? <a href='" . admin_url("/admin.php?page=ph_fedex_registration") . "' target='_BLANK'>Click here to Register</a>", "ph-fedex-woocommerce-shipping"); ?></strong></li>
						<li><?php _e("PluginHive, a FedEx Compatible Solutions provider, ensures a seamless FedEx account integration with the latest FedEx RESTful APIs for all merchants to upgrade your shipping & tracking experience.", "ph-fedex-woocommerce-shipping"); ?></li>
						<li></li>
	...
	public function generate_single_select_country_html() {
		...
		<tr valign="top" class="fedex_general_tab">
			<th scope="row" class="titledesc">
				<label for="origin_country"><?php _e( 'Origin Country and State', 'ph-fedex-woocommerce-shipping' ); ?></label>
			</th>
			<td class="forminp">
		...
		<tr valign="top" class="fedex_general_tab ph_fedex_alt_return_address">
			...
			<th scope="row" class="titledesc">
				<label for="woocommerce_wf_fedex_woocommerce_shipping_alt_return_country_state">
					<?php _e( 'Alternate Return Country', 'ph-fedex-woocommerce-shipping' ); ?>
	...
	public function generate_settings_tabs_html()
	{
		...
		echo '
			...
			$tabs = array(
				'general' 			=> __("General", 'ph-fedex-woocommerce-shipping'),
				'rates' 			=> __("Rates & Services", 'ph-fedex-woocommerce-shipping'),
				'labels' 			=> __("Label Generation", 'ph-fedex-woocommerce-shipping'),
				'commercial_invoice'=> __("International Forms", 'ph-fedex-woocommerce-shipping'),
				'special_services'	=> __("Special Services", 'ph-fedex-woocommerce-shipping'),
				'packaging' 		=> __("Packaging", 'ph-fedex-woocommerce-shipping'),
				'pickup' 			=> __("Pickup", 'ph-fedex-woocommerce-shipping'),
				'freight' 			=> __("Freight", 'ph-fedex-woocommerce-shipping'),
				'advanced' 			=> __("Advanced", 'ph-fedex-woocommerce-shipping'),
				'help_and_support ' => __("Help & Support", 'ph-fedex-woocommerce-shipping'),
			);
	...
	public function generate_services_html() {
		...
		// To show updated service list on saving settings
		$this->getServicesBasedOnOrigin();
		...
		ob_start();?>
			<tr style="padding-top: 0px;" class="fedex_general_tab">
				...
				<td style="vertical-align: top;padding-top: 0px;">
					<input type="button" value=" <?php echo __("Validate Credentials", 'ph-fedex-woocommerce-shipping'); ?>" id="xa_fedex_validate_credentials" class="button button-secondary" name="xa_fedex_validate_credentials" size="5" >
	...
	public function validate_box_packing_field( $key ) {
		...
		$selectedMetrics 	= ( isset( $_POST['selected_dim_unit'] ) && !empty( $_POST['selected_dim_unit'] ) ) ? $_POST['selected_dim_unit'] : $this->savedMetrics;
		...
		if( $selectedMetrics != $this->savedMetrics ) {
			$boxes = $this->ph_convert_box_measurement_units( $boxes );
		}

		return $boxes;
	}

	public function validate_fedex_email_format_field( $key ) {

		if ( isset( $_POST['ph_fedex_email_format'] ) )
			return str_replace('\\','',$_POST['ph_fedex_email_format']);
		return '';
	}
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			foreach( $additional_products as $values) {
				...
				if ( ! $values['data']->get_weight() ) {
					$this->debug( sprintf( __( 'FedEx - Product Weight Missing, Aborting! Product Id - %d, Product Name - %s.', 'ph-fedex-woocommerce-shipping' ), $values['data']->get_id(), $values['data']->get_name() ), 'error' );
...
/**
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			foreach( $additional_products as $values) {
				...
				if ( $dimensions[0] && $dimensions[1] && $dimensions[2] && $weight ) {	
					...
					$this->debug( sprintf( __( 'FedEx - Product Dimensions Missing, Aborting! Product Id - %d, Product Name - %s.', 'ph-fedex-woocommerce-shipping' ), $values['data']->get_id(), $values['data']->get_name() ), 'error' );
	...
	private function weight_based_shipping( $package ){
		...
		if( !empty($pre_packed_contents) ) {
			...
			$prepacked_requests = $this->wf_fedex_add_pre_packed_product( $pre_packed_contents,$group_id,$weightflag );
			$to_ship = array_merge($to_ship, $prepacked_requests);
		}
	...
	public function residential_address_validation( $package ) {
		...
		if ( !empty($package['destination']['address']) && !empty($package['destination']['country']) && in_array($package['destination']['country'],$this->address_validation_contries) ) {
			...
			try {
				...
				$wsdlPath = plugin_dir_path( dirname( __FILE__ ) ) . 'fedex-wsdl/production/AddressValidationService_v' . $this->addressvalidationservice_version. '.wsdl';
				...
				if(Ph_Fedex_Woocommerce_Shipping_Common::phIsNewRegistration())
				{
					...
					{
						$this->diagnostic_report( "----------------------------- FEDEX ADDRESS VALIDATION -----------------------------" );
						$this->diagnostic_report( "Please use a valid plugin license to continue using WooCommerce FedEx Shipping Plugin with Print Label" );
						return;
					} else {
						...
						$client = $this->wf_create_soap_client( $wsdlPath, $proxyParams['options'] );
					...
					$client = $this->wf_create_soap_client( $wsdlPath );
				}
				...
				if ( WF_FEDEX_ADV_DEBUG_MODE == "on" ) { // Test mode is only for development purpose.
			 	...
			 	$this->debug( 'FedEx ADDRESS VALIDATION REQUEST: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $request, true ) . '</pre>' );
			 	if( $exception_message !== null ) {
			 		$this->debug( 'FedEx ADDRESS VALIDATION Exception: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $exception_message, true ) . '</pre>' );
			 	...
			 	$this->debug( 'FedEx ADDRESS VALIDATION RESPONSE: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $response, true ) . '</pre>' );
				...
				$this->debug( 'FedEx ADDRESS VALIDATION: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' .'An Unexpected error occured while calling API.' . '</pre>' );
	...
	public function get_fedex_api_request( $package, $request_type = '' ) {
		...
		// Support Saturday Delivery modification using Checkout Editor Add-on
		if ( isset( $package['ph_fedex_sat_delivery'] ) ) {

			// Admin Settings modified by Customer chosen at checkout
			$this->saturday_delivery = $package['ph_fedex_sat_delivery'];

			if ( $this->saturday_delivery && $request_type == 'saturday_delivery' ) {
				
				// Modify the time stamp date, forced next Thursday date, when sat delivery option enabled at checkout and day is not Thursday or Friday
				if ( ( $timeStamptDay != 'Friday' ) && ( $timeStamptDay != 'Thursday' ) ) {

					$newTimeStampDate = $timeStampDate->modify('next Thursday');
					$request['RequestedShipment']['ShipTimestamp'] = $newTimeStampDate->format('c');
				}
			}
		}
		...
		$request['RequestedShipment']['Shipper']		   = array(
			'Address'			   => array(
				...
				'CountryCode'			 => isset( Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper[$package_origin['CountryCode']] ) ? Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper[$package_origin['CountryCode']] : $package_origin['CountryCode'],
			)
		);
		...
		$request['RequestedShipment']['Recipient'] = array(
			'Address' => array(
				...
				'StateOrProvinceCode' => strlen( $package['destination']['state'] ) == 2 ? strtoupper( $package['destination']['state'] ) : '',
				'CountryCode'		 => isset( Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper[$package['destination']['country']] ) ? Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper[$package['destination']['country']] : $package['destination']['country'],
				'StreetLines'		=> isset( $package['destination']['address_1'] ) ? $package['destination']['address_1'] : ''
			)
		);
	...
	public function skip_products( $package ) {
		...
		if( !empty( $skipped_products ) ) {
			...
			$this->diagnostic_report( 'Skipped Products Ids from Package' );
			...
			foreach ( $parcel_chunks as $parcels ) {
				...
				foreach ( $parcels as $key => $parcel ) {
					...
					if ( !empty( $order_no )) {
						...
						PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $order_no, 'ph_get_no_of_packages'.$parcel['GroupNumber'], $no_of_packages );
					}
					...
					for( $i=0; $i<$no_of_packages; $i++ ) { 
						...
						$this->packaging_type = $request['RequestedShipment']['PackagingType'];
						...
						if ( 'freight' === $request_type ) {
								...
								foreach ( $parcel_request['packed_products'] as $product ) {
									...
									for ($loop = 0; $loop < $parcel_request['GroupPackageCount']; $loop++) {
										...
										if ( $option == 'HAZARDOUS_MATERIALS' ) {
											...
											// Adding DANGEROUS_GOODS details in LineItem Description
							...
							if ( $parcel_request['packed_products'] ) {
								...
								foreach ( $parcel_request['packed_products'] as $pid => $product ) {
									...
									if( $is_dry_ice_product=='yes' ){
										...
										$dry_ice_total_weight += $this->round_up( Ph_Fedex_Woocommerce_Shipping_Common::ph_get_converted_weight( $dry_ice_weight, 'kg' )*$parcel['GroupPackageCount'], 2); //Fedex support dry ice weight in KG only
									}
									...
									$hst = $this->xa_get_custom_product_option_details( array( $parcel['packed_products'][$pid] ), '_wf_hs_code' );

									if ( ! empty( $hst ) ) {

										$commodoties[ $product->get_id() ]['HarmonizedCode'] = current( $hst );
							...
							if ($this->fedex_one_rate && ( '' === $request_type || $request_type == 'saturday_delivery') && in_array($request['RequestedShipment']['PackagingType'], $this->fedex_one_rate_package_ids) && count($parcels) == 1) {
						...
						if ( ! $this->insure_contents || 'smartpost' === $request_type || empty($parcel['InsuredValue']['Amount']) || ( isset($package['cart_subtotal']) && $this->min_order_amount_for_insurance > $package['cart_subtotal'] ) ) {
							
							unset( $parcel_request['InsuredValue'] );

							if ( $this->insure_contents && 'smartpost' != $request_type ) {

								$this->diagnostic_report(' Insurance eligibility criteria not met - Order subtotal must be greater or equal to Min Order Amount.');
							}
						}
						...
						if( ( isset($parcel_request['signature_option']) && !empty($parcel_request['signature_option'])) || ( isset( $_GET['signature_option'] ) ) ){
							...
							PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $order_ID, 'ph_fedex_signature_option_meta', $signature_option );
						}
						else{
						...
						if ( isset($_GET['sat_delivery']) ) {
							...
							PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $order_ID, 'ph_fedex_sat_delivery_option_meta', $_GET['sat_delivery'] );
						}
						...
						if ( isset( $signature_option ) && !empty( $signature_option ) ) {
							...
							if ( $request_type != 'freight' || ( $request_type == 'freight' && $signature_option == 'SERVICE_DEFAULT' ) ) {
							...
							if ( $request_type == 'freight' && !empty( $signature_option ) && $is_skiped_signature ) {

								$this->diagnostic_report( 'Not Eligible for Signature - Freight Shipment' );
								$this->diagnostic_report( print_r( $signature_option, 1) );
							}
						}
						...
						if ( isset( $dgOptionsType ) && !empty( $dgOptionsType ) && $request_type != 'freight' && ( $dgOptionsType == 'ORM_D' ) ) {
						...
						if ( isset( $dgOptionsType ) && !empty( $dgOptionsType ) && $request_type != 'freight' && ( $dgOptionsType == 'LIMITED_QUANTITIES_COMMODITIES' ) ) {
						...
						if ( isset( $dgOptionsType ) && !empty( $dgOptionsType ) && $request_type != 'freight' && ( $dgOptionsType == 'HAZARDOUS_MATERIALS' || $dgOptionsType == 'BATTERY' ) ) {
							...
							$SpecialServices['SpecialServiceTypes'][]			= 'DANGEROUS_GOODS';
							$SpecialServices['DangerousGoodsDetail']['Options']	= $dgOptionsType;
						...
						if( !empty($batteryProducts) ){
							...
							$SpecialServices['SpecialServiceTypes'][]	= 'BATTERY';

							foreach ( $batteryProducts as $productKey => $value) {

								$SpecialServices['BatteryDetails'] = array(
				...
				if ( 'smartpost' === $request_type ) {
					...
					$this->debug( __( 'Only $100 amount will be insured for smartpost.', 'ph-fedex-woocommerce-shipping' ) );

					$this->diagnostic_report( 'Only $100 amount will be insured for smartpost.' );

				} elseif ( $this->insure_contents && !empty($total_value) && isset($package['cart_subtotal']) && $this->min_order_amount_for_insurance <= $package['cart_subtotal'] ) {
				...
				if ( 'freight' === $request_type ) {
					...
					$request['RequestedShipment']['Shipper'] = array(
						...
						'Address'			=> array(
							...
							'CountryCode'			=> isset( Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper[strtoupper( $this->origin_country )] ) ? Ph_Fedex_Woocommerce_Shipping_Common::$ph_fedex_country_code_mapper[strtoupper( $this->origin_country )] : strtoupper( $this->origin_country ),
					...
					// Add SpecialServiceTypes DANGEROUS_GOODS in Shipment Level when Hazardous Materials In Package
	...
	*/
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( empty( $package['destination']['country'] ) ) {
			$this->debug( __('FedEx: Country not yet supplied. Rates not requested.', 'ph-fedex-woocommerce-shipping') );
			return; 
		}

		if( in_array( $package['destination']['country'] , $this->noPostcodeCountryArray ) ) {
			if ( empty( $package['destination']['city'] ) ) {
				$this->debug( __('FedEx: City not yet supplied. Rates not requested.', 'ph-fedex-woocommerce-shipping') );
				return;
			}
		}
		else if( empty( $package['destination']['postcode'] ) ) {
			$this->debug( __('FedEx: Zip not yet supplied. Rates not requested.', 'ph-fedex-woocommerce-shipping') );
			return;
		}
		...
		if(Ph_Fedex_Woocommerce_Shipping_Common::phIsNewRegistration())
		{
			...
			if(!Ph_Fedex_Woocommerce_Shipping_Common::phHasActiveLicense())
			{
				...
				$this->debug( __( 'Please use a valid plugin license to continue using WooCommerce FedEx Shipping Plugin with Print Label', 'ph-fedex-woocommerce-shipping' ), 'error' );
				return;
			}
		}
		...
		foreach( $packages as $pack_key => $package ) {
			...
			$this->destinationCountryCode = isset( $package['destination']['country'] ) ? $package['destination']['country'] : '';
			...
			$this->vendorId = isset($package['vendorID']) && !empty($package['vendorID']) ? $package['vendorID'] : null ;
			...
			if ( isset( $fedex_requests[$pack_key] ) ) {

				$fedex_requests[$pack_key]['vendorID'] = $this->vendorId;
			}
		}
		...
		if ( $fedex_requests ) {
			$this->run_package_request( $fedex_requests, 'normal_rates' );
		}

		// FedEx team mentioned that SmartPost does not support Multi-piece shipments.
		$is_single_package = false;

		if ( count($fedex_packages) == 1 && current($fedex_packages)['GroupPackageCount'] == 1 ) {
			
			$is_single_package = true;
		}
		
		if ( $is_single_package && !empty( $this->custom_services['SMART_POST']['enabled'] ) && ! empty( $this->smartpost_hub ) && $package['destination']['country'] == 'US' && ( $smartpost_requests = $this->get_fedex_requests( $fedex_packages, $package, 'smartpost' ) ) ) {
		...
		if( $this->saturday_delivery && ( $satdelivery_requests = $this->get_fedex_requests( $fedex_packages, $package, 'saturday_delivery' ) ) ) {
			...
			$this->run_package_request( $satdelivery_requests, 'saturday_delivery' );
		}
	...
	public function run_package_request( $requests, $type = '' ) {
		...
		try {
			foreach ( $requests as $key => $request ) {
				...
				if ( isset( $request['vendorID'])){
					...
					unset($request['vendorID']);
				}

				$this->process_result( $this->get_result( $request, $isNewAndActiveRegistration, $apiAccessDetails, $type ) );
			}
		} catch ( Exception $e ) {
	...
	private function get_result( $request, $isNewAndActiveRegistration = false, $apiAccessDetails = [], $type = '' ) {
		...
		if($isNewAndActiveRegistration)
		{
			...
			$client = $this->wf_create_soap_client( plugin_dir_path( dirname( __FILE__ ) ) . 'fedex-wsdl/' . ( $this->production ? 'production' : 'test' ) . '/RateService_v' . $this->rateservice_version. '.wsdl', $proxyParams['options'] );
		...
		if( $this->debug ) {
			...
			$this->debug( 'FedEx REQUEST: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $debug_request_to_display, true ) . '</pre>' );
			if( $exception_message !== null ) {
				$this->debug( 'FedEx RATE EXCEPTION: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $exception_message, true ) . '</pre>' );
			...
			$this->debug( 'FedEx RESPONSE: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $debug_response_to_display, true ) . '</pre>' );
			...
			try{
				$xml_request 	= $this->soap_method != 'nusoap' ? $client->__getLastRequest() : $client->request;
				$xml_response 	= $this->soap_method != 'nusoap' ? $client->__getLastResponse() : $client->response;			
				$this->debug( 'FedEx  REQUEST in XML Format: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( htmlspecialchars( $xml_request ), true ) . "</pre>\n" );
				$this->debug( 'FedEx RESPONSE in XML Format: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( htmlspecialchars( $xml_response ), true ) . "</pre>\n" );
				...
				$this->debug( 'Exception while getting FedEx Request or Response: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( $e->getMessage(), true ) . '</pre>' );
			}
		}
										...
										foreach( $data->RateReplyDetails as $quote ) {
											foreach( $quote->RatedShipmentDetails as $rate_details ) {
												...
												if( $this->request_type == "LIST" ) {
													if( isset($rate_details->ShipmentRateDetail) && strstr( $rate_details->ShipmentRateDetail->RateType, 'PAYOR_LIST' ) && empty($debug_data[$quote->ServiceType]['ListRates']) ) {
														$debug_data[$quote->ServiceType]['ListRates'] = $rate_details->ShipmentRateDetail->TotalNetCharge;
													}
													elseif( isset($rate_details->ShipmentRateDetail) && empty($debug_data[$quote->ServiceType]['AccountRates']) ){
	...
	private function process_result( $result = null ) {
		if ( $result && ! empty ( $result->RateReplyDetails ) ) {
			...
			foreach ( $rate_reply_details as $quote ) {
				...
				if ( is_array( $quote->RatedShipmentDetails ) ) {
					...
					if ( $this->request_type == "LIST" ) {
						...
						foreach ( $quote->RatedShipmentDetails as $i => $d ) {
							if ( isset($d->ShipmentRateDetail) && strstr( $d->ShipmentRateDetail->RateType, 'PAYOR_LIST' ) ) {
						...
						foreach ( $quote->RatedShipmentDetails as $i => $d ) {
							if ( isset($d->ShipmentRateDetail) && strstr( $d->ShipmentRateDetail->RateType, 'PAYOR_ACCOUNT' ) ) {
								$details = $quote->RatedShipmentDetails[ $i ];
								break;
							}
						}
				...
				// Remove FEDEX_INTERNATIONAL_PRIORITY and show INTERNATIONAL_PRIORITY if destination country is PR
				if( ( !isset($this->services[ $quote->ServiceType ]) && !( $this->destinationCountryCode == 'PR' && $quote->ServiceType == 'INTERNATIONAL_PRIORITY' ) ) || ( $quote->ServiceType == 'FEDEX_INTERNATIONAL_PRIORITY' && $this->destinationCountryCode == 'PR' ) ) {
					continue;
				}
				...
				// For service type INTERNATIONAL_PRIORITY rate_name will be empty so adding rate_name if destination country is PR
				if( ( $this->destinationCountryCode == 'PR' && $quote->ServiceType == 'INTERNATIONAL_PRIORITY' ) ) {
					$rate_name = 'FedEx International Priority®';
				}
				...
				if( $this->shipping_charge ==='DUTIES_AND_TAXES'&& isset($details->ShipmentRateDetail) && isset($details->ShipmentRateDetail->TotalNetChargeWithDutiesAndTaxes) && isset($details->ShipmentRateDetail->TotalNetChargeWithDutiesAndTaxes->Amount) )
				{
					$rate_cost = floatval( $details->ShipmentRateDetail->TotalNetChargeWithDutiesAndTaxes->Amount );

				}elseif( $this->shipping_charge ==='NET_FEDEX_CHARGE'&& isset($details->ShipmentRateDetail) && isset($details->ShipmentRateDetail->TotalNetFedExCharge) && isset($details->ShipmentRateDetail->TotalNetFedExCharge->Amount) )
				{
					$rate_cost = floatval( $details->ShipmentRateDetail->TotalNetFedExCharge->Amount );

				}elseif( $this->shipping_charge ==='BASE_CHARGE'&& isset($details->ShipmentRateDetail) && isset($details->ShipmentRateDetail->TotalBaseCharge) && isset($details->ShipmentRateDetail->TotalBaseCharge->Amount) )
				{
	...
	private function convert_to_base_currency($details,$rate_cost){
		...
		if($this->convert_currency_to_base == 'yes'){
			if( isset($details->ShipmentRateDetail) && property_exists($details->ShipmentRateDetail,'CurrencyExchangeRate')){
	...
	private function prepare_rate( $rate_code, $rate_id, $rate_name, $rate_cost, $rate_name_extra='' ) {
		...
		$this->conversion_rate = apply_filters('ph_vendor_conversion_rate', $this->conversion_rate , $this->vendorId, 'fedex');

		if ( $this->conversion_rate ) {
			
			$rate_cost *= $this->convert_to_store_currency($this->conversion_rate);
		}
		...
		if( $this->found_rates && isset($_GET['wf_fedex_generate_packages_rates']) && !empty($_GET['oid']) ) {
			PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data( $_GET['oid'], 'wf_fedex_generate_packages_rates_response', $this->found_rates );
		}
	}
	...
	public function add_found_rates() {
		...
		if ( $this->found_rates ) {
			...
			$this->debug( __('FedEx: Using Fallback Setting.', 'ph-fedex-woocommerce-shipping') );
