...
class PH_FedEx_Help_and_Support {
	...
	public function ph_get_fedex_log_data() {	
		...
		if( class_exists('WC_Log_Handler_File') ){
			...
			$log_file_name	= strtolower(PH_FEDEX_DEBUG_LOG_FILE_NAME) . '-' . date( 'Y-m-d' );

			$is_log_available	= false;
			foreach ( $all_log_file_list as $log_file ) {
				if ( str_contains( strtolower( $log_file ), $log_file_name ) ) {
			...
			if( !empty($log_file_name) && !empty($all_log_file_list) && $is_log_available ) {

				$log_file_path = ABSPATH.$log_directory.$log_file_name;
