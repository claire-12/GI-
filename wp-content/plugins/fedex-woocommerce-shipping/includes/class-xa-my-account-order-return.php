...
class xa_my_account_order_return
{
	...
	public $debug;
	/**
	 * Return Label Dom Service
	 */
	public $retun_label_dom_service;
	/**
	 * Return Label Int Service
	 */
	public $retun_label_int_service;
	/**
	 * Frontend return Label
	 */
	public $frontend_retun_label;
	/**
	 * Return Label Reason Required
	 */
	public $return_label_reason_required;
	...
	public function xa_return_from_my_account_form( $order ) {
		...
		$shipmentIds = PH_WC_Fedex_Storage_Handler::ph_get_meta_data($this->order_id, 'wf_woo_fedex_shipmentId', false);
		if (!empty($shipmentIds)) {
			echo "<h2>Shipping details</h2>";
			foreach($shipmentIds as $shipmentId) {
				$shipping_return_label = PH_WC_Fedex_Storage_Handler::ph_get_meta_data($this->order_id, 'wf_woo_fedex_returnLabel_'.$shipmentId);
				$return_shipment_id = PH_WC_Fedex_Storage_Handler::ph_get_meta_data($this->order_id, 'wf_woo_fedex_returnShipmetId');
				if(!empty($shipping_return_label)){
					...
					<a class="button tips" href="<?php echo $download_url; ?>" data-tip="<?php _e('Print Return Label', 'ph-fedex-woocommerce-shipping'); ?>"><?php _e('Print Return Label', 'ph-fedex-woocommerce-shipping'); ?></a>
					</li>
					<?php 
				}else{
					...
					if( $this->return_label_reason_required ) {
						...
						<button type='button' class='fedex_create_return_shipment' onclick=create_return_label("<?php echo $shipmentId ?>","<?php echo base64_encode($shipmentId.'|'.$this->order_id) ?>") data-tip="<?php _e( 'Generate return label', 'ph-fedex-woocommerce-shipping' ); ?>"> Generate Return Label </button>
						<?php
					}
					else {
						?>
						<a class="button button-primary fedex_create_return_shipment tips" href="<?php echo $generate_url?>" data-tip="<?php _e( 'Generate return label', 'ph-fedex-woocommerce-shipping' ); ?>"><?php _e( 'Generate return label', 'ph-fedex-woocommerce-shipping' ); ?></a>
						<?php
					}
					?>
					</li><?php
	...
	public function generate_fedex_return_label(){
		...
		$return_label	= PH_WC_Fedex_Storage_Handler::ph_get_meta_data($this->order_id, 'wf_woo_fedex_returnLabel_'.$shipment_id);
		...
		if( ! empty($return_label) ) {
			if( ! headers_sent() && ! $this->debug ) {
				if( function_exists('wc_add_notice') ){
					wc_add_notice( __( 'FedEx Return Label has already been generated for this Shipment ', 'ph-fedex-woocommerce-shipping' ).print_r($shipment_id,true) );
				}
				wp_redirect($view_current_order_page_link);
			}
			else{
				echo __( 'FedEx Return Label has already been generated for this Shipment ', 'ph-fedex-woocommerce-shipping' ).print_r($shipment_id,true);
				echo "<br/><br/><a href='$view_current_order_page_link'><button class='ph_go_back_button_my_account_page'>".__( 'Go Back', 'ph-fedex-woocommerce-shipping' )."</button></a>";
			}
		}
		else{
			...
			if( ! empty($_GET['fedex_return_reason']) ) {
				...
				$order->add_order_note( __('Reason for return of shipment '). $shipment_id .' : '.mb_convert_encoding($_GET['fedex_return_reason'], "UTF-8"), 0, 1 );
			...
			if( ! $this->debug ){
				if( ! headers_sent() ){
					...
					echo "<a href='$view_current_order_page_link'><button class='ph_go_back_button_my_account_page'>".__( 'Go Back', 'ph-fedex-woocommerce-shipping' )."</button></a>";
				}
			}
			else{
				echo "<a href='$view_current_order_page_link'><button class='ph_go_back_button_my_account_page'>".__( 'Go Back', 'ph-fedex-woocommerce-shipping' )."</button></a>";
	...
	private function wf_get_shipping_service($order,$retrive_from_order = false, $shipment_id=false){
		...
		$order = wc_get_order($this->order_id);
		$this->is_international = ( $order->get_shipping_country() != $origin_country ) ? true : false;
		...
		if($retrive_from_order == true){
			$service_code = PH_WC_Fedex_Storage_Handler::ph_get_meta_data($order->get_id(), 'wf_woo_fedex_service_code'.$shipment_id);
