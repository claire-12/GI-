...
class Ph_FedEx_Tracking_Util {
	...
	public static function prepare_shipment_source_data( $order_id, $shipment_id_cs, $shipping_service, $order_date ){
		...
		if( !empty( $order->get_shipping_postcode() ) ) {
			$shipment_source_data['shipping_postcode'] = $order->get_shipping_postcode();
		}
		else {
	...
	public static function update_tracking_data ( $order_id, 
													...
													$shipment_result_key, 
													$order_date='' ) {
		$shipment_source_data = PH_WC_Fedex_Storage_Handler::ph_get_meta_data($order_id, $shipment_source_key);
		...
		PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, $shipment_source_key, $shipment_source_data);
		...
		if ( null != $shipment_result && is_object( $shipment_result ) ) {
			...
			PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, $shipment_result_key, $shipment_result_array);
			$message = self::get_shipment_display_message( $shipment_result_array, $shipment_source_data );
		}
		else {
			$message = __( 'Unable to update tracking info.', 'ph-fedex-woocommerce-shipping' );
			PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, $shipment_result_key, '');
		}
