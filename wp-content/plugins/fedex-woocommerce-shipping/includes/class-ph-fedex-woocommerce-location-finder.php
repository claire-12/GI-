...
if( ! class_exists('Ph_Fedex_Woocommerce_Location_Finder') ) {
    class Ph_Fedex_Woocommerce_Location_Finder {
         ...
         * Admin Details
         */
        public $account_number, $meter_number;
        /**
         * Mode
         */
        public $production;
         ...
         * attribute Variables
         */
        public $attribute_type, $location_attributes;
        /**
         * Soap Method
         */
        public $soap_method;
        /**
         * Location service Version
         */
        public $location_service_version;
        ...
        public function __construct($settings=array()) {
            ...
            $this->request_hash     = '';
            $this->attribute_type	= ( isset( $this->fedex_settings['attribute_type'] ) && !empty( $this->fedex_settings['attribute_type'] ) ) ? $this->fedex_settings['attribute_type'] : 'all';
            $this->location_attributes = ( isset( $this->fedex_settings['location_attributes'] ) && !empty($this->fedex_settings['location_attributes'] ) ) ? $this->fedex_settings['location_attributes'] : '';
        ...
        public function fedex_on_hold_locations() {
            ...
            if ( isset($_POST['country']) && !empty( $_POST['country']) && ( $this->fedex_settings['availability'] == 'all' || ( $this->fedex_settings['availability'] == 'specific' && in_array( $_POST['country'], $this->fedex_settings['countries'] ) ) ) ) {

                $request = $this->get_request();
            }

            if( ! empty($request) ) {
                $result = $this->get_result($request, $isNewAndActiveRegistration, $apiAccessDetails);
         ...
         * @return object.
         */
        public function get_result( $request, $isNewAndActiveRegistration, $apiAccessDetails )
        {
            $result = null;
            ...
            // Search Location WSDL version
            $this->location_service_version = '12';   
            ...
            if($isNewAndActiveRegistration)
		    {
                ...
                $client = $this->ph_create_soap_client( plugin_dir_path( dirname( __FILE__ ) ) . 'fedex-wsdl/'. ( $this->production ? 'production' : 'test' ) .'/LocationsService_v'.$this->location_service_version.'.wsdl', $proxyParams['options'] );
            ...
            try{
                ...
                $this->debug( 'FedEx Hold at Location Request: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( htmlspecialchars( $xml_request ), true ) . "</pre>\n" );
                $this->debug( 'FedEx Hold at Location Response: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#EEE;color:#000;border:1px solid #DDD;padding:5px;">' . print_r( htmlspecialchars( $xml_response ), true ) . "</pre>\n" );

                Ph_Fedex_Woocommerce_Shipping_Common::addAdminDiagnosticReport('--------------------------- HOLD AT LOCATION REQUEST ---------------------------');
                Ph_Fedex_Woocommerce_Shipping_Common::addAdminDiagnosticReport(print_r($xml_request,1));
                Ph_Fedex_Woocommerce_Shipping_Common::addAdminDiagnosticReport('--------------------------- HOLD AT LOCATION RESPONSE ---------------------------');
                Ph_Fedex_Woocommerce_Shipping_Common::addAdminDiagnosticReport(print_r($xml_response,1));

            }
            catch( Exception $e ){
                $this->debug( __('Something Went Wrong while Fetching Search Loactions Request & Response', 'ph-fedex-woocommerce-shipping') );
            }
         ...
         * @return object.
         */
        public function ph_create_soap_client( $wsdl, $options = ['trace' => true] ){
                ...
                foreach( $cart as $cart_item) {
                    ...
                    if( !$product->is_virtual() ) {
                        ...
                        $fields['billing']['ph_fedex_hold_at_locations'] = array(
                            'label'       => __('FedEx Hold at Location', 'ph-fedex-woocommerce-shipping'),
                            'placeholder' => _x('', 'placeholder', 'ph-fedex-woocommerce-shipping'),
                            'required'    => false,
                            'clear'       => false,
                            'type'        => 'select',
                            'priority'      => 200,
                            ...
                            'options'     => array(
                                '' => __('Select FedEx Hold at Location', 'ph-fedex-woocommerce-shipping' )
                            )
                        );
                        break;
                    }
        ...
        public function ph_fedex_update_fedex_hold_at_location_select_options( $array ){
            ...
            $locator    .=  "<option value=''>". __('Select FedEx Hold at Location', 'ph-fedex-woocommerce-shipping') ."</option>";
        ...
        public function ph_fedex_update_hold_at_location_data_in_rate_request( $request , $parcels) {
            ...
            else // 133908 - Order Edit Page - Rate Request
            {
                ...
                if(!empty($order) && is_object($order))
                {
                    ...
                    $selected_fedex_hold_at_location = PH_WC_Fedex_Storage_Handler::ph_get_meta_data($order_id, 'ph_fedex_hold_at_location');
                }
            }
        ...
        */
        ...
        public function ph_fedex_update_hold_at_location_in_order_meta( $order_id ,$data) {
            ...
            if( ! empty($selected_fedex_hold_at_location) ) {
                ...
                PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, 'ph_fedex_hold_at_location', $selected_fedex_hold_at_location);
                ...
                PH_WC_Fedex_Storage_Handler::ph_update_and_save_meta_data($order_id, 'ph_fedex_selected_hold_at_location', $selectedAddress);
            }
        }
        ...
        public function ph_fedex_formatted_address_replacements( $formatted_address, $address ) {
            $hold_at_location_tag = ! empty($address['ph_fedex_hold_at_location']) ? __( 'Hold At Location:  ', 'ph-fedex-woocommerce-shipping' ).$address['ph_fedex_hold_at_location'] :'';
        ...
        {
            $order_id = $order->get_id();
            $ph_fedex_hold_at_location = PH_WC_Fedex_Storage_Handler::ph_get_meta_data($order_id, 'ph_fedex_hold_at_location');

            if( !empty($ph_fedex_hold_at_location) ) {
                    $contact=array();
