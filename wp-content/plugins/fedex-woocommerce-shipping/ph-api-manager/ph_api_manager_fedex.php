...
if ( ! class_exists( 'PH_FedEx_API_Manager' ) ) {
	class PH_FedEx_API_Manager {
		...
		// To support FedEx Registration
		public $ph_key						= '';
		public $ph_prev_api_key_key         = '';
		public $ph_product_order_api_key    = '';

		public $ph_fedex_api_history;
		public $ph_api_history_tab_key		  = '';
		public $ph_menu_tab_api_history_title  = '';
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '' ) {
			...
			if ( is_admin() ) {
				...
				$this->ph_key            			= 'ph_client_fedex';
				$this->ph_prev_api_key_key  		= $this->ph_key . '_prev_api_key';
				$this->ph_product_order_api_key 	= $this->ph_key . '_product_order_api_key';

				$this->ph_api_history_tab_key         = $this->data_key . '_api_history';
				$this->ph_menu_tab_api_history_title  = esc_html__( 'API History', $this->text_domain );
				...
				if ( !class_exists('PH_FedEx_Api_History_In_Database') ) {

					include_once('ph_fedex_api_history_in_database.php');
				}

				$this->ph_fedex_api_history = new PH_FedEx_Api_History_In_Database();
			}
		...
		public function config_page() {
			...
			if ( $this->ph_fedex_api_history->ph_fedex_api_history_table_exists() ) {

				$settings_tabs[$this->ph_api_history_tab_key] = esc_html__( $this->ph_menu_tab_api_history_title, $this->text_domain );
			}
            ...
            <div class='wrap'>
				...
				<p><a href="https://www.pluginhive.com/knowledge-base/setting-woocommerce-fedex-shipping-plugin/#Activate_License" target="_BLANK"><?php _e( "How to Activate the plugin license?", "ph-fedex-woocommerce-shipping" ); ?></a></p>
				<p><?php _e( "If you are still facing difficulties, please follow the below steps:", "ph-fedex-woocommerce-shipping" ); ?></p>
				<ul class="ph_api_key_activation_steps">
					<li><p><?php _e( "Go to <a href='https://www.pluginhive.com/my-account/api-keys/' target='_BLANK'>PluginHive > My Account > API Keys</a> Section", "ph-fedex-woocommerce-shipping" ); ?></p></li>
					<li><p><?php _e( "Delete the previous activation of the license key (If any)", "ph-fedex-woocommerce-shipping" ); ?></p></li>
					<li><p><?php _e( "Deactivate and reactivate the FedEx plugin", "ph-fedex-woocommerce-shipping" ); ?></p></li>
					<li><p><?php _e( "Verify the API key details again and activate the API Key by clicking Save Changes", "ph-fedex-woocommerce-shipping" ); ?></p></li>
					<li><p><?php _e( "If you are still facing difficulties, kindly contact <a href='https://www.pluginhive.com/support/' target='_BLANK'>PluginHive Support</a> with the WordPress Administrator Access, along with the PluginHive Purchase Order Details. We will activate it on your site promptly.", "ph-fedex-woocommerce-shipping" ); ?></p></li>
				</ul>
                ...
                <form action='options.php' method='post'>
                    <div class="main">
						...
						if ( $tab == $this->wc_am_activation_tab_key ) {
						...
						} else if ( $tab == $this->wc_am_deactivation_tab_key ) {
							...
							$this->ph_fedex_api_history->ph_fetch_fedex_api_history_table_data();
						}
						?>
                    </div>
                </form>
		...
		public function validate_options( $input ) {
			...
			// Send existing Product Order API Key in Activate Request
			$ph_prev_api_key  					 = get_option( $this->ph_prev_api_key_key );
			...
			if ( $_REQUEST[ 'option_page' ] != $this->wc_am_deactivate_checkbox_key ) {
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					$args = array(
						'api_key' => $api_key,
						'previousLicenseKey' 	=> $ph_prev_api_key,
					);
					...
					if( isset( $activate_results[ 'data' ] ) && isset( $activate_results[ 'data' ][ 'product_order_api_key' ] ) ) {
						update_option( $this->ph_prev_api_key_key, $activate_results[ 'data' ][ 'product_order_api_key' ] );
						update_option( $this->ph_product_order_api_key, $activate_results[ 'data' ][ 'product_order_api_key' ] );

						do_action('ph_wc_fedex_plugin_license_activated', $activate_results['data']['product_order_api_key']);
						
						$this->ph_fedex_api_history->ph_insert_data_to_fedex_api_history_table($activate_results);
					}
		...
		public function wc_am_license_key_deactivation( $input ) {
			...
			if ( $options == 'on' && $activation_status == 'Activated' && isset( $this->data[ $this->wc_am_api_key_key ] ) && !empty( $this->data[ $this->wc_am_api_key_key ] ) ) {
